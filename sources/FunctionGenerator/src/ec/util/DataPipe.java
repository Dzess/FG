...
public class DataPipe {
	...
	// the number of bytes in the array -- in reality this is the number of
	// bytes written to the array so far
	int size = 0;
	...
	/**
	 * Reset the buffer. Does not resize it back to a smaller size -- if it has
	 * ballooned it will stay large, though it will no longer have wasted space
	 * in it. If you wish to make the buffer a more manageable size, create a
	 * new DataPipe instead.
	 */
	public void reset() {
		pull = size = 0;
	}
	...
	/**
	 * Returns the number of elements written to the buffer so far (after the
	 * last reset()).
	 */
	...
	/**
	 * Returns the number of elements read from the buffer so far (after the
	 * last reset()).
	 */
	public int numRead() {
		return pull;
	 ...
	 * an object to the pipe, then deserializes it.
	 */
