 ...
 * (unlike in a Prototype, where the prototype instance is never used, but only
 * makes clones which are used). When a new Group instance is needed, it is
 * created by calling emptyClone() on a previous Group instance, which returns a
 * new instance set up exactly like the first Group instance had been set up
 * when setup(...) was called on it.
 * 
 * Groups are Serializable and Cloneable, but you should not clone them -- use
 * emptyClone instead.
 * 
 * 
 * @author Sean Luke
