 ...
 * 
 * <p>
 * Slave is the main entry point for a slave evaluation process. The slave works
 * with a master process, receiving individuals from the master, evaluating
 * them, and reporting the results back to the master, thus enabling distributed
 * evolution.
 * 
 * <p>
 * Slave replicates most of the functionality of the ec.Evolve class, for
 * example in terms of parameters and checkpointing. This is mostly because it
 * needs to bootstrap and set up the EvolutionState in much the same way that
 * ec.Evolve does. Additionally, depending on settings below, the Slave may act
 * like a mini-evolver on the individuals it receives from the master.
 * 
 * <p>
 * Like ec.Evolve, Slave is run with like this:
 * 
 * <p>
 * <tt>java ec.eval.Slave -file </tt><i>parameter_file [</i><tt>-p </tt>
 * <i>parameter=value]*</i>
 * 
 * <p>
 * This starts a new slave, using the parameter file <i>parameter_file</i>. The
 * user can provide optional overriding parameters on the command-line with the
 * <tt>-p</tt> option.
 * 
 * <p>
 * Slaves need to know some things in order to run: the master's IP address and
 * socket port number, whether to do compression, and whether or not to return
 * individuals or just fitnesses. Unfortunately, Sun's
 * CompressedInputStream/CompressedOutputStream is broken (it doesn't allow
 * partial flushes, which is critical for doing compressed network streams). In
 * order to do compression, you need to download the JZLIB library from the ECJ
 * website or from http://www.jcraft.com/jzlib/ . ECJ will detect and use it
 * automatically.
 * 
 * <p>
 * Slaves presently always run in single-threaded mode and receive their random
 * number generator seed from the master. Thus they ignore any seed parameters
 * given to them.
 * 
 * <p>
 ...
 * <ul>
 * <p>
 * <li>"Regular" mode, which does a loop where it receives N individuals,
 * evaluates them, and returns either the individuals or their new fitnesses.
 * <p>
 * <li>"Regular Coevolutionary" mode, which does a loop where it receives N
 * individuals to assess together in a single coevolutionary evaluation,
 * evaluates them, and returns either the individuals or their new fitnesses (or
 * only some fitnesses if only some are requested).
 * <p>
 * <li>"Evolve" mode, which does a loop where it receives N individuals,
 * evaluates them, and if there's some more time left, does a little evolution
 * on those individuals as if they were a population, then when the time is up,
 * the current individuals in the population are returned in lieu of the
 * original individuals. In this second form, individuals MUST be returned, not
 * fitnesses. This mode is not available if you're doing coevolution.
 * </ul>
 * 
 * <p>
 * <b>Parameters</b><br>
 ...
 * 
 * <tr>
 * <td valign=top><tt>eval.slave-name</tt><br>
 * <font size=-1> String </font></td>
 * <td valign=top>(the slave's name, only for debugging purposes. If not
 * specified, the slave makes one up.)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><tt>eval.master.host</tt><br>
 * <font size=-1> String </font></td>
 * <td valign=top>(the IP Address of the master.)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><tt>eval.master.port</tt><br>
 * <font size=-1> integer &gt;= 1024 </font></td>
 * <td valign=top>(the socket port number of the master.)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><tt>eval.compression</tt><br>
 * <font size=-1> bool = <tt>true</tt> or <tt>false</tt> (default) </font></td>
 * <td valign=top>(should we use compressed streams in communicating with the
 * master?)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><tt>eval.run-evolve</tt><br>
 * <font size=-1> bool = <tt>true</tt> or <tt>false</tt> (default) </font></td>
 * <td valign=top>(should we immediately evaluate the individuals and return
 * them (or their fitnesses), or if we have extra time (defined by
 * eval.runtime), should we do a little evolution on our individuals first?)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><tt>eval.runtime</tt><br>
 * <font size=-1> integer &gt; 0 </font></td>
 * <td valign=top>(if eval.run-evolve is true, how long (in milliseconds
 * wall-clock time) should we allow the individuals to evolve?)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><tt>eval.return-inds</tt><br>
 * <font size=-1> bool = <tt>true</tt> or <tt>false</tt> (default) </font></td>
 * <td valign=top>(should we return whole individuals or (if false) just the
 * fitnesses of the individuals? This must be TRUE if eval.run-evolve is true.)</td>
 * </tr>
 * 
 * <!--
 * <tr>
...
public class Slave {
	...
	/**
	 * How long we sleep in between attempts to connect to the master (in
	 * milliseconds).
	 */
	...
	public static void main(String[] args) {
		...
		for (x = 0; x < args.length - 1; x++)
			if (args[x].equals(A_FILE)) {
				try {
					...
					// this is used only by the Evaluator to determine whether
					// to use the MasterProblem
					parameters.set(new Parameter(ec.EvolutionState.P_EVALUATOR).push(ec.Evaluator.P_IAMSLAVE), "true");
					break;
				} catch (FileNotFoundException e) {
					Output.initialError("A File Not Found Exception was generated upon" + "reading the parameter file \"" + args[x + 1]
							+ "\".\nHere it is:\n" + e);
				} catch (IOException e) {
					Output.initialError("An IO Exception was generated upon reading the" + "parameter file \"" + args[x + 1]
							+ "\".\nHere it is:\n" + e);
				}
			}
		if (parameters == null)
		...
		// 5. Determine whether or not to return entire Individuals or just
		// Fitnesses
		...
		int masterPort = parameters.getInt(new Parameter(P_EVALMASTERPORT), null);
			...
			try {
				...
				try {
					...
					if (useCompression) {
						...
						/*
						 * tmpIn = new CompressingInputStream(tmpIn); tmpOut =
						 * new CompressingOutputStream(tmpOut);
						 */
				...
				// 1. create the output
				// store = parameters.getBoolean(new Parameter(P_STORE), null,
				// false);
				...
				/*
				 * int breedthreads = parameters.getInt( new
				 * Parameter(Evolve.P_BREEDTHREADS),null,1);
				 * 
				 * if (breedthreads < 1) output.fatal(
				 * "Number of breeding threads should be an integer >0.", new
				 * Parameter(Evolve.P_BREEDTHREADS),null);
				 * 
				 * 
				 * int evalthreads = parameters.getInt( new
				 * Parameter(Evolve.P_EVALTHREADS),null,1);
				 * 
				 * if (evalthreads < 1)
				...
				// Note that either breedthreads or evalthreads (or both) may be
				// 'auto'. We don't warn about this because
				...
				for (int i = 0; i < random.length; i++)
					random[i] = Evolve.primeGenerator(new MersenneTwisterFast(seed++)); // we
																						// prime
																						// the
																						// generator
																						// to
				...
				try {
					while (true) {
						...
						if (runEvolve) {
							// Construct and use a new EvolutionState. This will
							// be inefficient the first time around
							// as we've set up TWO EvolutionStates in a row with
							// no good reason.
							ParameterDatabase coverDatabase = new ParameterDatabase(); // protect
																						// the
																						// underlying
																						// one
					...
					// Since an IOException can happen here if the peer closes
					// the socket
					// on it's end, we don't necessarily have to exit. Maybe we
					// don't
					// even need to print a warning, but we'll do so just to
					// indicate
					// something happened.
					state.output
							.warning("Unable to read type of evaluation from master.  Maybe the master closed its socket and exited?:\n"
									+ e);
				}
	...
	public static void evaluateSimpleProblemForm(EvolutionState state, boolean returnIndividuals, DataInputStream dataIn,
			DataOutputStream dataOut, String[] args) {
		...
		int[] indsPerSubpop = new int[state.population.subpops.length]; // num
																		// inds
																		// for
																		// each
																		// subpop
		...
		if (runEvolve) {
			...
			// Now we need to reset the subpopulations. They were already set up
			// with the right
			// classes, Species, etc. in state.setup(), so all we need to do is
			// modify the number
	...
	public static void evaluateGroupedProblemForm(EvolutionState state, boolean returnIndividuals, DataInputStream dataIn,
	...
	private static void returnIndividualsToMaster(EvolutionState state, Individual[] inds, boolean[] updateFitness,
		...
		for (int i = 0; i < inds.length; i++) {
			...
			// System.err.println("writing byte: " + ( returnIndividuals ?
			// V_INDIVIDUAL : (updateFitness[i] ? V_FITNESS : V_NOTHING)));
