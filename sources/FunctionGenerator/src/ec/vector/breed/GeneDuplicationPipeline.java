...
/**
 * <p>
 * GeneDuplicationPipeline is designed to duplicate a sequence of genes from the
 * chromosome and append them to the end of the chromosome. The sequence of
 * genes copied are randomly determined. That is to say a random begining index
 * is selected and a random ending index is selected from the chromosome. Then
 * this area is then copied (begining inclusive, ending exclusive) and appended
 * to the end of the chromosome. Since randomness is a factor several checks are
 * performed to make sure the begining and ending indicies are valid. For
 * example, since the ending index is exclusive, the ending index cannot equal
 * the begining index (a new ending index would be randomly seleceted in this
 * case). Likewise the begining index cannot be larger than the ending index
 * (they would be swapped in this case).
 * </p>
 * 
 * <p>
 * <b>Default Base</b><br>
...
public class GeneDuplicationPipeline extends BreedingPipeline {
	...
	public int produce(int min, int max, int start, int subpopulation, Individual[] inds, EvolutionState state, int thread) {
		...
		for (int q = start; q < n + start; q++) {
			...
			} while (begin == end); // because the end is exclusive, start
									// cannot be
			// equal to end.
			...
			// clone the genes in splice[1] (which we'll concatenate back in) in
			// case we're using GeneVectorIndividual
