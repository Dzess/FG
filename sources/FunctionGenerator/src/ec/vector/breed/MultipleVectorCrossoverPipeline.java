...
/**
 * 
 MultipleVectorCrossoverPipeline is a BreedingPipeline which implements a
 * uniform (any point) crossover between multiple vectors. It is intended to be
 * used with three or more vectors. It takes n parent individuals and returns n
 * crossed over individuals. The number of parents and consequently children is
 * specified by the number of sources parameter.
 * <p>
 * The standard vector crossover probability is used for this crossover type. <br>
 * <i> Note</i> : It is necessary to set the crossover-type parameter to 'any'
...
public class MultipleVectorCrossoverPipeline extends BreedingPipeline {
...

	{
		...
		if (inds[0] instanceof BitVectorIndividual)
			n = multipleBitVectorCrossover(min, max, start, subpopulation, // redundant
																			// reassignment
					inds, state, thread);

		else if (inds[0] instanceof ByteVectorIndividual)
			n = multipleByteVectorCrossover(min, max, start, subpopulation, inds, state, thread);

		else if (inds[0] instanceof DoubleVectorIndividual)
			n = multipleDoubleVectorCrossover(min, max, start, subpopulation, inds, state, thread);

		else if (inds[0] instanceof FloatVectorIndividual)
			n = multipleFloatVectorCrossover(min, max, start, subpopulation, inds, state, thread);

		else if (inds[0] instanceof IntegerVectorIndividual)
			n = multipleIntegerVectorCrossover(min, max, start, subpopulation, inds, state, thread);

		else if (inds[0] instanceof GeneVectorIndividual)
			n = multipleGeneVectorCrossover(min, max, start, subpopulation, inds, state, thread);

		else if (inds[0] instanceof LongVectorIndividual)
			n = multipleLongVectorCrossover(min, max, start, subpopulation, inds, state, thread);

		else if (inds[0] instanceof ShortVectorIndividual)
			n = multipleShortVectorCrossover(min, max, start, subpopulation, inds, state, thread);

		else // default crossover -- shouldn't need this unless a new vector
				// type is added
		{
			...
			for (int i = 0; i < pieces[0].length; i++) {
				if (state.random[thread].nextBoolean(species.crossoverProbability)) {
					// shuffle
					for (int j = pieces.length - 1; j > 0; j--) // no need to
																// shuffle first
																// index at the
																// end
					{
						// find parent to swap piece with
						int parent2 = state.random[thread].nextInt(j); // not
																		// inclusive;
																		// don't
																		// want
																		// to
			...
			// join them and add them to the population starting at the start
			// location
	 ...
	 */
	public int multipleBitVectorCrossover(final int min, final int max, final int start, final int subpopulation, final Individual[] inds,
			final EvolutionState state, final int thread) {
		if (!(inds[0] instanceof BitVectorIndividual))
													...
													// sources.length
		{
			// produce one parent from each source
			sources[i].produce(1, 1, i, subpopulation, parents, state, thread);
			if (!(sources[i] instanceof BreedingPipeline)) // it's a selection
															// method probably
				parents[i] = (BitVectorIndividual) (parents[i].clone());
		}

		VectorSpecies species = (VectorSpecies) inds[0].species; // doesn't
																	// really
																	// matter if
		...
		for (int i = 0; i < parents[0].genomeLength(); i++) {
			if (state.random[thread].nextBoolean(species.crossoverProbability)) {
				for (int j = parents.length - 1; j > 0; j--) {
					int swapIndex = state.random[thread].nextInt(j); // not
																		// inclusive;
																		// don't
																		// want
																		// to
																		...
																		// with
																		// self
					boolean temp = ((BitVectorIndividual) parents[j]).genome[i]; // modifying
																					// genomes
																					// directly.
																					// it's
																					// okay
																					...
																					// they're
																					// clones
					((BitVectorIndividual) parents[j]).genome[i] = ((BitVectorIndividual) parents[swapIndex]).genome[i];
	...
	/**
	 * Crosses over the Byte Vector Individuals using a uniform crossover
	 * method.
	 * 
	 * There is no need to call this method separately; produce(...) calls it
	 * whenever necessary by default.
	 */
	public int multipleByteVectorCrossover(final int min, final int max, final int start, final int subpopulation, final Individual[] inds,
			final EvolutionState state, final int thread) {
			...
			if (state.random[thread].nextBoolean(species.crossoverProbability)) {
				for (int j = parents.length - 1; j > 0; j--) {
					int swapIndex = state.random[thread].nextInt(j); // not
																		...
																		// with
																		// self
					byte temp = ((ByteVectorIndividual) parents[j]).genome[i]; // modifying
																				// genomes
																				// directly.
																				// it's
																				// okay
	...
	/**
	 * Crosses over the Double Vector Individuals using a uniform crossover
	 * method.
	 * 
	 * There is no need to call this method separately; produce(...) calls it
	 * whenever necessary by default.
	 */
	public int multipleDoubleVectorCrossover(final int min, final int max, final int start, final int subpopulation,
			...
			if (state.random[thread].nextBoolean(species.crossoverProbability)) {
				for (int j = parents.length - 1; j > 0; j--) {
					int swapIndex = state.random[thread].nextInt(j); // not
																		...
																		// with
																		// self
					double temp = ((DoubleVectorIndividual) parents[j]).genome[i]; // modifying
																					// genomes
																					// directly.
																					// it's
																					// okay
	...
	/**
	 * Crosses over the Float Vector Individuals using a uniform crossover
	 * method.
	 * 
	 * There is no need to call this method separately; produce(...) calls it
	 * whenever necessary by default.
	 */
	public int multipleFloatVectorCrossover(final int min, final int max, final int start, final int subpopulation,
			final Individual[] inds, final EvolutionState state, final int thread) {
			...
			if (state.random[thread].nextBoolean(species.crossoverProbability)) {
				for (int j = parents.length - 1; j > 0; j--) {
					int swapIndex = state.random[thread].nextInt(j); // not
																		...
																		// with
																		// self
					float temp = ((FloatVectorIndividual) parents[j]).genome[i]; // modifying
																					// genomes
																					// directly.
																					// it's
																					// okay
	...
	/**
	 * Crosses over the Gene Vector Individuals using a uniform crossover
	 * method.
	 * 
	 * There is no need to call this method separately; produce(...) calls it
	 * whenever necessary by default.
	 */
	public int multipleGeneVectorCrossover(final int min, final int max, final int start, final int subpopulation, final Individual[] inds,
			final EvolutionState state, final int thread) {
			...
			if (state.random[thread].nextBoolean(species.crossoverProbability)) {
				for (int j = parents.length - 1; j > 0; j--) {
					int swapIndex = state.random[thread].nextInt(j); // not
																		...
																		// with
																		// self
					VectorGene temp = ((GeneVectorIndividual) parents[j]).genome[i]; // modifying
																						// genomes
																						// directly.
																						// it's
																						// okay
	...
	/**
	 * Crosses over the Integer Vector Individuals using a uniform crossover
	 * method.
	 * 
	 * There is no need to call this method separately; produce(...) calls it
	 * whenever necessary by default.
	 */
	public int multipleIntegerVectorCrossover(final int min, final int max, final int start, final int subpopulation,
			final Individual[] inds, final EvolutionState state, final int thread) {
			...
			if (state.random[thread].nextBoolean(species.crossoverProbability)) {
				for (int j = parents.length - 1; j > 0; j--) {
					int swapIndex = state.random[thread].nextInt(j); // not
																		...
																		// with
																		// self
					int temp = ((IntegerVectorIndividual) parents[j]).genome[i]; // modifying
																					// genomes
																					// directly.
																					// it's
																					// okay
	...
	/**
	 * Crosses over the Long Vector Individuals using a uniform crossover
	 * method.
	 * 
	 * There is no need to call this method separately; produce(...) calls it
	 * whenever necessary by default.
	 */
	public int multipleLongVectorCrossover(final int min, final int max, final int start, final int subpopulation, final Individual[] inds,
			final EvolutionState state, final int thread) {
			...
			if (state.random[thread].nextBoolean(species.crossoverProbability)) {
				for (int j = parents.length - 1; j > 0; j--) {
					int swapIndex = state.random[thread].nextInt(j); // not
																		...
																		// with
																		// self
					long temp = ((LongVectorIndividual) parents[j]).genome[i]; // modifying
																				// genomes
																				// directly.
																				// it's
																				// okay
	...
	/**
	 * Crosses over the Short Vector Individuals using a uniform crossover
	 * method.
	 * 
	 * There is no need to call this method separately; produce(...) calls it
	 * whenever necessary by default.
	 */
	public int multipleShortVectorCrossover(final int min, final int max, final int start, final int subpopulation,
			final Individual[] inds, final EvolutionState state, final int thread) {
			...
			if (state.random[thread].nextBoolean(species.crossoverProbability)) {
				for (int j = parents.length - 1; j > 0; j--) {
					int swapIndex = state.random[thread].nextInt(j); // not
																		...
																		// with
																		// self
					short temp = ((ShortVectorIndividual) parents[j]).genome[i]; // modifying
																					// genomes
																					// directly.
																					// it's
																					// okay
