...
public class IntegerVectorSpecies extends VectorSpecies {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (maxGene < minGene)
			state.output.fatal("IntegerVectorSpecies must have a default min-gene which is <= the default max-gene", base.push(P_MAXGENE),
					def.push(P_MAXGENE));
			...
			if (!state.parameters.exists(base.push(P_MINGENE).push("" + x), base.push(P_MINGENE).push("" + x))) {
				if (foundStuff && !warnedMin) {
					state.output.warning("IntegerVectorSpecies has missing min-gene values for some genes.\n" + "The first one is gene #"
							+ x + ".", base.push(P_MINGENE).push("" + x), base.push(P_MINGENE).push("" + x));
					warnedMin = true;
				}
			} else {
				if (dynamicInitialSize)
				...
				minGenes[x] = state.parameters.getLongWithDefault(base.push(P_MINGENE).push("" + x), base.push(P_MINGENE).push("" + x),
						minGene);
				foundStuff = true;
			}
			...
			if (!state.parameters.exists(base.push(P_MAXGENE).push("" + x), base.push(P_MAXGENE).push("" + x))) {
				if (foundStuff && !warnedMax) {
					state.output.warning("IntegerVectorSpecies has missing max-gene values for some genes.\n" + "The first one is gene #"
							+ x + ".", base.push(P_MAXGENE).push("" + x), base.push(P_MAXGENE).push("" + x));
					warnedMax = true;
				}
			} else {
				if (dynamicInitialSize)
				...
				maxGenes[x] = state.parameters.getLongWithDefault(base.push(P_MAXGENE).push("" + x), base.push(P_MAXGENE).push("" + x),
						maxGene);
				foundStuff = true;
			}
		}
	...
	private void initializeGenomeSegmentsByStartIndices(final EvolutionState state, final Parameter base, final Parameter def,
		...
		for (int i = numSegments - 1; i >= 0; i--) {
			...
			{
				currentSegmentMinGeneValue = state.parameters.getLongWithDefault(base.push(P_SEGMENT).push("" + i).push(P_MINGENE), base
			...
			{
				currentSegmentMaxGeneValue = state.parameters.getLongWithDefault(base.push(P_SEGMENT).push("" + i).push(P_MAXGENE), base
