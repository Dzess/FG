 ...
 * VectorGenes. The default mutation method calls the mutate() method on each
 * gene independently with <tt>species.mutationProbability</tt>. Initialization
 * calls reset(), which should call reset() on each gene. Do not expect that the
 * genes will actually exist during initialization -- see the default
 * implementation of reset() as an example for how to handle this.
 * 
 * 
 * <P>
