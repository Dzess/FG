...
/**
 * BitVectorIndividual is a VectorIndividual whose genome is an array of
 * booleans. The default mutation method simply flips bits with
 * <tt>mutationProbability</tt>.
 * 
 * <P>
...
public class BitVectorIndividual extends VectorIndividual {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		VectorSpecies s = (VectorSpecies) species; // where my default info is
													// stored
	 ...
	 * form does a bit-flip with a probability depending on parameters.
	 */
	...
	public void writeGenotype(final EvolutionState state, final DataOutput dataOutput) throws IOException {
		...
		for (int x = 0; x < genome.length; x++)
			dataOutput.writeBoolean(genome[x]); // inefficient: booleans are
