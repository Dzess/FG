...
public class SimpleEvaluator extends Evaluator {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (!(p_problem instanceof SimpleProblemForm))
			state.output.fatal("" + this.getClass() + " used, but the Problem is not of SimpleProblemForm", base.push(P_PROBLEM));
	}
		...
		for (int y = 0; y < state.evalthreads; y++)
			for (int x = 0; x < state.population.subpops.length; x++) {
				...
				if (y < state.evalthreads - 1) // not last one
					...
					(state.population.subpops[x].individuals.length - (state.population.subpops[x].individuals.length / state.evalthreads) // note
																																			// integer
																																			// division
							* state.evalthreads);
