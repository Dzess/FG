	...
	public void setupConstraints(final EvolutionState state, final Parameter base) {
		...
		for (int y = 0; y < x; y++) {
			...
			if (state.parameters.exists(base.push("" + y), null))
				...
				state.output.message("No Rule Constraints specified, assuming the default class: ec.rule.RuleConstraints for "
						+ base.push("" + y));
	...
	public void setupRuleSetConstraints(final EvolutionState state, final Parameter base) {
		...
		for (int y = 0; y < x; y++) {
			...
			if (state.parameters.exists(base.push("" + y), null))
				...
				state.output.message("No RuleSetConstraints specified, assuming the default class: ec.gp.RuleSetConstraints for "
						+ base.push("" + y));
