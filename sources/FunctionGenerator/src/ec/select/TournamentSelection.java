 ...
 * 
 * <p>
 * <i>size</i> can also be a floating-point value between 1.0 and 2.0, exclusive
 * of them. In this situation, two individuals are chosen at random, and the
 * better one is selected with a probability of <i>size/2</i>
 * 
 * <p>
...
public class TournamentSelection extends SelectionMethod implements SteadyStateBSourceForm {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		else if (val == (int) val) // easy, it's just an integer
		{
			...
			probabilityOfPickingSizePlusOne = val - size; // for example, if we
															// have 5.4, then
															// the probability
															// of picking *6* is
															// 0.4
	...
	public int getTournamentSizeToUse(MersenneTwisterFast random) {
		double p = probabilityOfPickingSizePlusOne; // pulls us to under 35
													// bytes
		if (p == 0.0)
			return size;
	...
	public int produce(final int subpopulation, final EvolutionState state, final int thread) {
		...
		if (pickWorst)
			for (int x = 1; x < s; x++) {
				...
				if (!betterThan(oldinds[j], oldinds[best], subpopulation, state, thread)) // j
																							// is
																							// at
																							// least
																							// as
