...
public class ADF extends GPNode {
		...
		if (associatedTree < 0 || associatedTree > typicalIndividual.trees.length)
					...
					if (gpfi[x] instanceof ADFArgument) {
						...
						if (arg >= children.length) // uh oh
							state.output.error("The node " + toStringForError() + " in individual " + individualBase
									+ " would call its associated tree, which has an argument terminal with an argument number (" + arg
									+ ") >= the ADF/ADM's arity (" + children.length + ").  The argument terminal in question is "
									+ gpfi[x].toStringForError());
						else {
							if (validArgument[arg] != null && validArgument[arg] != argument) // got
																								// one
																								// already
								state.output.warning("There exists more than one Argument terminal for argument #" + arg + " for the node "
			...
			for (int x = 0; x < children.length; x++)
				if (validArgument[x] == null)
					state.output.warning("There is no argument terminal for argument #" + x + " for the node " + toStringForError()
							+ " in individual " + individualBase);
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (associatedTree < 0)
			state.output.fatal("ADF/ADM node must have a positive-numbered associated tree.", base.push(P_ASSOCIATEDTREE),
		...
		if (name.length() == 1) {
			state.output.warning(
					"Using old-style ADF/ADM name.  You should change it to something longer and more descriptive, such as ADF" + name,
					base.push(P_FUNCTIONNAME), def.push(P_FUNCTIONNAME));
		}
