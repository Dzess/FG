...
public final class GPSetType extends GPType {
	...
	public void setup(final EvolutionState state, Parameter base) {
		...
		for (int x = 0; x < len; x++) {
			...
			if (s == null)
				state.output.fatal("Atomic type member #" + x + " is not defined for the GPSetType " + name + ".", base.push(P_MEMBER)
						.push("" + x));
			GPType t = GPType.typeFor(s, state);
			if (!(t instanceof GPAtomicType)) // uh oh
				state.output.fatal("Atomic type member #" + x + " of GPSetType " + name + " is not a GPAtomicType.", base.push(P_MEMBER)
						.push("" + x));
