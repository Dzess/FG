...
/**
 * A GrammarParser is the basic class for parsing a GE ruleset into a parse
 * graph of GrammarNodes. This parse graph is then later used to produce a
 * GPIndividual from a GEIndividual in GESpecies. It is assumed that the root
 * will represent the first rule given in the grammar.
 * 
 */
...
public class GrammarParser implements Prototype {
	...
	/**
	 * The default regular expressions for tokens in the parser. If you'd like
	 * to change minor features of the regular expressions, override the
	 * getRegexes() method in a subclass to return a different array. Note that
	 * if you INSERT a new regular expression into the middle of these, the
	 * values of the various token constants ("LPAREN", "RULE", etc.) will be
	 * wrong, so you will need to override or modify the methods which use them.
	 */
	public static final String[] DEFAULT_REGEXES = new String[] { "\\p{Blank}*#[^\\n\\r]*", // COMMENT:
			...
			"\\p{Blank}*<[^<>()\\p{Space}]*>", // RULE: matches a rule of the
												// form <foo>. No <, >, (, ), |,
												// or spaces may appear in foo
			"\\p{Blank}*[|]", // PIPE: matches a |
			"\\p{Blank}*::=", // EQUALS: matches a :==
			"\\p{Blank}*::=", // NUMERIC_CONSTANT: does nothing right now, so
								// set to be identical to EQUALS. Reserved for
								// future use.
			"\\p{Blank}*::=", // BOOLEAN_CONSTANT: does nothing right now, so
								// set to be identical to EQUALS. Reserved for
								// future use.
			"\\p{Blank}*::=", // STRING_CONSTANT: does nothing right now, so set
								// to be identical to EQUALS. Reserved for
								// future use.
			"\\p{Blank}*[^<>()|\\p{Space}]+", // FUNCTION (must appear after
												// RULE and PIPE): matches a
												// rule of the form foo. No <,
												// >, (, ), |, or spaces may
												// appear in foo, and foo must
	...
	/**
	 * Returns the regular expressions to use for tokenizing these rules. By
	 * default DEFAULT_REGEXES are returned.
	 */
	...
	// Returns a rule from the hashmap. If one does not exist, creates a rule
	// with the
	...
	// Parses a rule, one rule per line, from the lexer. Adds to the existing
	// hashmap if there's already a rule there.
	GrammarRuleNode parseRule(EvolutionState state, Lexer lexer, GPFunctionSet gpfs) {
		...
		if (lexer.getMatchingIndex() == COMMENT)
		...
		//
		// Don't merge into any existing rule -- I do that in parseRules below.
		// Instead, just pull out
		// rules and hang them into your "new rule" as necessary.
		// Use getRule(rules, "<rulename>") to extract the rule representing the
		// current rule name which you
		// can hang inside there as necessary.
		//
		// If you have to you can call state.output.fatal(...) which will
		// terminate the program,
		// but piling up some errors might be useful. I'll handle the
		// exitIfErors() in parseRules below
		//
		// Return null if there was no rule to parse (blank line or all
		// comments) but no errors.
	...
	void parseProductions(EvolutionState state, GrammarRuleNode retResult, Lexer lexer, GPFunctionSet gpfs) {
		...
		do {
			...
			if (lexer.getMatchingIndex() == RULE) {
				...
				token = lexer.nextToken();
				if (lexer.getMatchingIndex() != FUNCTION) // now expecting
															// function
				{
					state.output.fatal("GE Grammar Error - Expecting a function name after first '(' for rule: " + retResult.getHead()
							+ " Error: " + token);
				} else {
					...
					while (lexer.getMatchingIndex() != RPAREN) {
						if (lexer.getMatchingIndex() != RULE) // this better be
																// the name of a
																// rule node
						{
							state.output.fatal("GE Grammar Error - Expecting a rule name as argument for function definition: "
	...
	/**
	 * Parses the rules from a grammar and returns the resulting GrammarRuleNode
	 * root.
	 */
	...
	/**
	 * Checks that all grammar rules in ruleshashmap have at least one possible
	 * production
	 * 
