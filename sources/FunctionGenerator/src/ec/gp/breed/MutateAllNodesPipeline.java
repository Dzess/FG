 ...
 * in Kumar Chellapilla, "A Preliminary Investigation into Evolving Modular
 * Programs without Subtree Crossover", GP98.
 * 
 * <p>
 * MutateAllNodesPipeline chooses a subtree and for each node <i>n</i> in that
 * subtree, it replaces <i>n</i> with a randomly-picked node of the same arity
 * and type constraints. Thus the original topological structure is the same but
...
public class MutateAllNodesPipeline extends GPBreedingPipeline {
	...
	private GPNode pickCompatibleNode(final GPNode original, final GPFunctionSet set, final EvolutionState state, final GPType returntype,
			final int thread) {
		...
		if (initializer.numAtomicTypes + initializer.numSetTypes == 1) // easy
																			...
																			// len
			{
				...
				for (int y = 0; y < set.nodesByArity[type][len][x].constraints(initializer).childtypes.length; y++)
					if (!set.nodesByArity[type][len][x].constraints(initializer).childtypes[y].compatibleWith(initializer,
							original.children[y].constraints(initializer).returntype)) {
						failed = true;
						break;
					}
				if (!failed)
	 ...
	 * created by making nodes "compatible" with the equivalent nodes in the
	 * tree rooted at original. You need to set the parent and argumentposition
	 * of the root yourself.
	 */

	private GPNode generateCompatibleTree(final GPNode original, final GPFunctionSet set, final EvolutionState state,
		...
		for (int x = 0; x < node.children.length; x++) {
			node.children[x] = generateCompatibleTree(original.children[x], set, state, original.constraints(initializer).childtypes[x],
					thread);
	...
	public int produce(final int min, final int max, final int start, final int subpopulation, final Individual[] inds,
		...
		for (int q = start; q < n + start; q++) {
			...
			if (tree != TREE_UNFIXED && (tree < 0 || tree >= i.trees.length))
				// uh oh
				state.output
						.fatal("MutateAllNodesPipeline attempted to fix tree.0 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual");
