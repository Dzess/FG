...
/**
 * MutateSwapPipeline works very similarly to the Swap algorithm described in
 * Kumar Chellapilla, "A Preliminary Investigation into Evolving Modular
 * Programs without Subtree Crossover", GP98.
 * 
 * <p>
 * MutateSwapPipeline picks a random tree, then picks randomly from all the
 * swappable nodes in the tree, and swaps two of its subtrees. If its chosen
 * tree has no swappable nodes, it repeats the choose-tree process. If after
 * <i>tries</i> times it has failed to find a tree with swappable nodes, it
 * gives up and simply copies the individual.
 * 
 * <p>
 * "Swapping" means to take a node <i>n</i>, and choose two children nodes of
 * <i>n</i>, <i>x</i> and <i>y</i>, such that <i>x</i>'s return type is
 * swap-compatible with <i>y</i>'s slot, and <i>y</i>'s return type is
 * swap-compatible with <i>x</i>'s slot. The subtrees rooted at <i>x</i> and
 * <i>y</i> are swapped.
 * 
 * <p>
 * A "Swappable" node means a node which is capable of swapping given the
 * existing function set. In general to swap a node <i>foo</i>, it must have at
 * least two children whose return types are type-compatible with each other's
 * slots in <i>foo</i>.
 * 
 * <p>
 * This method is very expensive in searching nodes for "swappability". However,
...
public class MutateSwapPipeline extends GPBreedingPipeline {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (numTries == 0)
			state.output.fatal("MutateSwapPipeline has an invalid number of tries (it must be >= 1).", base.push(P_NUM_TRIES),
	...
	/**
	 * This very expensive method (for types) might be improved in various ways
	 * I guess.
	 */

	private boolean swappable(final GPInitializer initializer, final GPNode node) {
	...
	private void swapSomething(final GPNode node, final EvolutionState state, final int thread) {
		if (((GPInitializer) state.initializer).numAtomicTypes + ((GPInitializer) state.initializer).numSetTypes == 1) // typeless
			...
			_swapSomething(node, state, thread);
	}
			...
			for (int y = x + 1; y < node.constraints(initializer).childtypes.length; y++)
				if (node.children[x].constraints(initializer).returntype.compatibleWith(initializer,
					...
					if (numSwappable == swapItem) // found it
					{
						...
						node.children[x].argposition = (byte) x;
						node.children[y].argposition = (byte) y;
						// no need to set parent -- it's the same parent of
						// course
						return;
					} else
						numSwappable++;
	...
	private int numSwappableNodes(final GPInitializer initializer, final GPNode root, int soFar) {
	...
	// sticks the node in
	private int pickSwappableNode(final GPInitializer initializer, final GPNode root, int num) {
		if (swappable(initializer, root)) {
			num--;
			if (num == -1) // found it
			{
	...
	public int produce(final int min, final int max, final int start, final int subpopulation, final Individual[] inds,
		...
		for (int q = start; q < n + start; q++) {
			...
			if (tree != TREE_UNFIXED && (tree < 0 || tree >= i.trees.length))
				// uh oh
				state.output
						.fatal("MutateSwapPipeline attempted to fix tree.0 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual");
