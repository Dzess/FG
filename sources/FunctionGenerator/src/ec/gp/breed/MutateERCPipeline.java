...
/**
 * MutateERCPipeline works very similarly to the "Gaussian" algorithm described
 * in Kumar Chellapilla, "A Preliminary Investigation into Evolving Modular
 * Programs without Subtree Crossover", GP98.
 * 
 * <p>
 * MutateERCPipeline picks a random node from a random tree in the individual,
 * using its node selector. It then proceeds to "mutate" every ERC (ephemeral
 * random constant) located in the subtree rooted at that node. It does this by
 * calling each ERC's <tt>mutateERC()</tt> method. The default form of
 * <tt>mutateERC()</tt> method is to simply call <tt>resetNode()</tt>, thus
 * randomizing the ERC; you may want to override this default to provide more
 * useful mutations, such as adding gaussian noise.
 * 
 * <p>
