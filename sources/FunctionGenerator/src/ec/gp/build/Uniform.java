...
public class Uniform extends GPNodeBuilder {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		useTrueDistribution = state.parameters.getBoolean(base.push(P_TRUEDISTRIBUTION), def.push(P_TRUEDISTRIBUTION), false);
		...
		if (minSize > 0) // we're using maxSize and minSize
			...
			state.output.fatal("Uniform is used for the GP node builder, but no distribution was specified."
					+ "  You must specify either a min/max size, or a full size distribution.", base.push(P_MINSIZE), def.push(P_MINSIZE));
		// preprocess offline
	...
	public BigInteger numTreesOfType(final GPInitializer initializer, final int functionset, final int type, final int size) {
	...
	public BigInteger numTreesRootedByNode(final GPInitializer initializer, final int functionset, final GPNode node, final int size) {
	...
	public BigInteger numChildPermutations(final GPInitializer initializer, final int functionset, final GPNode parent, final int size,
	...
	GPNode createTreeOfType(final EvolutionState state, final int thread, final GPInitializer initializer, final int functionset,
			final int type, final int size, final MersenneTwisterFast mt)

	{
		// System.out.println("" + functionset + " " + type + " " + size);
		int choice = RandomChoice
				.pickFromDistribution(ROOT_D[functionset][type][size], ROOT_D[functionset][type][size][0], mt.nextDouble());
		...
		if (size > 1) // nonterminal
			fillNodeWithChildren(state, thread, initializer, functionset, node, ROOT_D[functionset][type][size][choice].node, 0, size - 1,
					mt);
		return node;
	}

	void fillNodeWithChildren(final EvolutionState state, final int thread, final GPInitializer initializer, final int functionset,
			final GPNode parent, final GPNode parentc, final int pickchild, final int outof, final MersenneTwisterFast mt)

	{
		if (pickchild == parent.children.length - 1) {
			...
			int size = RandomChoice.pickFromDistribution(CHILD_D[functionset][intForNode(parentc)][outof][pickchild], mt.nextDouble());
			parent.children[pickchild] = createTreeOfType(state, thread, initializer, functionset,
