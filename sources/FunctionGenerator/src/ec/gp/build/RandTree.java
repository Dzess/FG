...
public class RandTree extends GPNodeBuilder {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (!canPick())
			state.output.fatal("RandTree requires some kind of size distribution set, either with " + P_MINSIZE + "/" + P_MAXSIZE
	...
	// recursive function to work out all combinations and push them onto
	// ArrayList
	...
	// This function parses the dyck word and puts random nodes into their
	// slots.
	private GPNode buildTree(final EvolutionState state, final int thread, final GPNodeParent parent, final int argposition,
			final GPFunctionSet set, final String dyckWord) {
		int counter = 0;
		Stack s = new Stack();
		char nextChar;
