...
/**
 * PTCFunctionSet is a GPFunctionSet which adheres to PTCFunctionSetForm, and
 * thus can be used with the PTC1 and PTC2 methods. Terminal and nonterminal
 * probabilities for nodes used in this function set are determined by the
 * <tt>prob</tt> parameter for the nodes' GPNodeConstraints object. That's not
 * the greatest solution, because it could require making a lot of different
 * GPNodeConstraints, customized for each node, but it's the best I can do for
 * now.
 * 
...
public class PTCFunctionSet extends GPFunctionSet implements PTCFunctionSetForm {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (allZeros)
			state.output
					.warning(
							"In this function set, the probabilities of all nonterminal functions have a 0.0 selection probability -- this will cause them all to be selected uniformly.  That could be an error.",
							base);
		allZeros = false;
		...
		if (allZeros)
			state.output
					.warning(
							"In this function set, the probabilities of all terminal functions have a 0.0 selection probability -- this will cause them all to be selected uniformly.  That could be an error.",
							base);

		if (!allOnes && !noOnes)
			state.output
					.warning(
							"In this function set, there are some functions with a selection probability of 1.0, but not all of them.  That could be an error.",
							base);

		// set up our node probabilities. Allow all zeros.
		for (int x = 0; x < q_ty.length; x++) {
			if (q_ty[x].length == 0)
				state.output.warning("Function Set " + name + " has no terminals for type number " + x
						+ ".  This may cause problems for you.");
			else
				RandomChoice.organizeDistribution(q_ty[x], true);
			if (q_ny[x].length == 0)
				state.output.warning("Function Set " + name + " has no nonterminals for type number " + x
						+ ".  This may cause problems for you.");
			else
	...
	public float[] computeNonterminalSelectionProbabilities(final int expectedTreeSize) {
		...
		for (int x = 0; x < q_ny.length; x++) {
			...
			for (int y = 0; y < q_ny[x].length; y++)
				count += (y == 0 ? q_ny[x][y] : q_ny[x][y] - q_ny[x][y - 1]) // it's
																				// organized
