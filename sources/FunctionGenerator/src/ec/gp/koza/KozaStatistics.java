...
public class KozaStatistics extends Statistics implements SteadyStateStatisticsForm {
	...
	public void postEvaluationStatistics(final EvolutionState state) {
		...
		for (int x = 0; x < state.population.subpops.length; x++) {
			...
			if (doFull) {
				...
				state.output.println("Avg Nodes: " + ((double) totNodesPerGen) / state.population.subpops[x].individuals.length,
						statisticslog);
			...
			state.output.print("Mean fitness raw: " + meanStandardized + " adjusted: " + meanAdjusted + " hits: " + ((double) hits)
	...
	public void finalStatistics(final EvolutionState state, final int result) {
		...
		if (doFull) {
			...
			state.output.println("Initialization: " + ((float) initializationTime) / 1000 + " secs total, " + nodesInitialized + " nodes, "
					+ nodesInitialized / (((float) initializationTime) / 1000) + " nodes/sec", statisticslog);
			...
			state.output.println("Breeding: " + ((float) breedingTime) / 1000 + " secs total, " + nodesBred + " nodes, " + nodesBred
					/ (((float) breedingTime) / 1000) + " nodes/sec", statisticslog);
			...
			state.output.println("Breeding: " + ((float) breedingUsage) / 1024 + " KB total, " + nodesBred + " nodes, " + nodesBred
					/ (((float) breedingUsage) / 1024) + " nodes/KB", statisticslog);
		}
