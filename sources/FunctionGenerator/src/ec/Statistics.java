...
/**
 * Statistics and its subclasses are Cliques which generate statistics during
 * the run. Statistics are arranged in a tree hierarchy -- The root statistics
 * object may have "children", and when the root is called, it calls its
 * children with the same message. You can override this behavior however you
 * see fit.
 * 
 * <p>
 * There are lots of places where statistics might be nice to print out. These
 * places are implemented as hooks in the Statistics object which you can
 * override if you like; otherwise they call the default behavior. If you plan
 * on allowing your Statistics subclass to contain children, you should remember
 * to call the appropriate super.foo() if you override any foo() method.
 * 
 * <p>
 * While there are lots of hooks, various EvolutionState objects only implement
 * a subset of them. You'll need to look at the EvolutionState code to see which
 * ones are implemented to make sure that your statistics hooks are called
 * appropriately!
 * 
 * <p>
 * Statistics objects should set up their statistics logs etc. during
 * <tt>setup(...)</tt>. Remember to make the log restartable in case of
 * restarting from a checkpoint.
 * 
 * <p>
 * <b>Steady-State Statistics</b>. For convenience, Statistics contains default
 * implementations of the SteadyStateStatisticsForm methods but does not
 * implement SteadyStateStatisticsForm. This mostly is intended to keep
 * SteadyStateStatistcsForm implementations from having to call functions on all
 * their children: instead they can just call foo.super();
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>num-children</tt><br>
 * <font size=-1>int &gt;= 0</font></td>
 * <td valign=top>(number of child statistics objects)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>child</tt>.<i>n</i><br>
 * <font size=-1>classname, inherits or equals ec.Statistics</font></td>
 * <td valign=top>(the class of child statistics object number <i>n</i>)</td>
 * </tr>
 * </table>
 * 
 * <p>
 ...
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>child</tt>.<i>n</i></td>
 * <td>species (child statistics object number <i>n</i>)</td>
 * </tr>
 * </table>
 * 
 * 
...
public class Statistics implements Singleton {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (t < 0)
			state.output.fatal("A Statistics object cannot have negative number of children", base.push(P_NUMCHILDREN));
	...
	/**
	 * Called immediately after the run has completed. <i>result</i> is either
	 * <tt>state.R_FAILURE</tt>, indicating that an ideal individual was not
	 * found, or <tt>state.R_SUCCESS</tt>, indicating that an ideal individual
	 * <i>was</i> found.
	 */
	...
	/**
	 * STEADY-STATE: called when a given Subpopulation is entering the
	 * Steady-State.
	 */
	...
	/**
	 * STEADY-STATE: called each time new individuals are bred during the
	...
	/**
	 * STEADY-STATE: called each time new individuals are evaluated during the
	 * steady-state process. You can look up the individuals in
	 * state.newIndividuals[]
	 */
	public void individualsEvaluatedStatistics(SteadyStateEvolutionState state, Individual[] newIndividuals, Individual[] oldIndividuals,
