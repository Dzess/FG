...
/**
 * A BreedingPipeline is a BreedingSource which provides "fresh" individuals
 * which can be used to fill a new population. BreedingPipelines might include
 * Crossover pipelines, various Mutation pipelines, etc. This abstract class
 * provides some default versions of various methods to simplify matters for
 * you. It also contains an array of breeding sources for your convenience. You
 * don't have to use them of course, but this means you have to customize the
 * default methods below to make sure they get distributed to your special
 * sources. Note that these sources may contain references to the same object --
 * they're not necessarily distinct. This is to provide both some simple DAG
 * features and also to conserve space.
 * 
 * 
 * <p>
 * A BreedingPipeline implements SteadyStateBSourceForm, meaning that it
 * receives the individualReplaced(...) and sourcesAreProperForm(...) messages.
 ...
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>num-sources</tt><br>
 * <font size=-1>int &gt;= 1</font></td>
 * <td valign=top>(User-specified number of sources to the pipeline. Some
 * pipelines have hard-coded numbers of sources; others indicate (with the java
 * constant DYNAMIC_SOURCES) that the number of sources is determined by this
 * user parameter instead.)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>source.</tt><i>n</i><br>
 * <font size=-1>classname, inherits and != BreedingSource, or the value
 * <tt>same</tt><br>
 * <td valign=top>(Source <i>n</i> for this BreedingPipeline. If the value is
 * set to <tt>same</tt>, then this source is the exact same source object as
 * <i>base</i>.<tt>source.</tt><i>n-1</i>, and further parameters for this
 * object will be ignored and treated as the same as those for <i>n-1</i>.
 * <tt>same<tt> is not valid for 
...
public abstract class BreedingPipeline extends BreedingSource implements SteadyStateBSourceForm {
	...
	/**
	 * Returns the "typical" number of individuals produced -- by default this
	 * is the minimum typical number of individuals produced by any children
	 * sources of the pipeline. If you'd prefer something different, override
	 * this method.
	 */
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (likelihood < 0.0f || likelihood > 1.0f)
			state.output.fatal("Breeding Pipeline likelihood must be a value between 0.0 and 1.0 inclusive", base.push(P_LIKELIHOOD),
		...
		for (int x = 0; x < sources.length; x++) {
			...
			if (s != null && s.equals(V_SAME)) {
				...
				sources[x] = (BreedingSource) (state.parameters.getInstanceForParameter(p, d, BreedingSource.class));
	...
	/**
	 * Performs direct cloning of n individuals. if produceChildrenFromSource is
	 * true, then
	 */
	public int reproduce(final int n, final int start, final int subpopulation, final Individual[] inds, final EvolutionState state,
	...
	public boolean produces(final EvolutionState state, final Population newpop, final int subpopulation, int thread) {
		for (int x = 0; x < sources.length; x++)
			if (x == 0 || sources[x] != sources[x - 1])
	...
	public void finishProducing(final EvolutionState state, final int subpopulation, final int thread) {
		for (int x = 0; x < sources.length; x++)
			if (x == 0 || sources[x] != sources[x - 1])
	...
	public void sourcesAreProperForm(final SteadyStateEvolutionState state) {
		for (int x = 0; x < sources.length; x++)
			if (!(sources[x] instanceof SteadyStateBSourceForm)) {
				state.output.error("The following breeding source is not of SteadyStateBSourceForm.", mybase.push(P_SOURCE).push("" + x),
						defaultBase().push(P_SOURCE).push("" + x));
			} else
