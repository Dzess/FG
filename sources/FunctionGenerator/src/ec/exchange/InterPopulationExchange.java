...
public class InterPopulationExchange extends Exchanger {
	...
	public void setup(final EvolutionState state, final Parameter _base) {
		...
		for (int i = 0; i < numsubpops; i++) {
			...
			exchangeInformation[i].immigrantsSelectionMethod = (SelectionMethod) state.parameters.getInstanceForParameter(
					p.push(P_SELECT_METHOD), base.push(P_SELECT_METHOD), ec.SelectionMethod.class);
			...
			if (state.parameters.exists(p.push(P_SELECT_TO_DIE_METHOD), base.push(P_SELECT_TO_DIE_METHOD)))
				exchangeInformation[i].indsToDieSelectionMethod = (SelectionMethod) state.parameters.getInstanceForParameter(
						p.push(P_SELECT_TO_DIE_METHOD), base.push(P_SELECT_TO_DIE_METHOD), ec.SelectionMethod.class);
			else
				// use RandomSelection
	...
	public Population postBreedingExchangePopulation(EvolutionState state) {
		...
		for (int x = 0; x < nImmigrants.length; x++) {
			...
			if (nImmigrants[x] > 0 && chatty) {
				state.output.message("Immigrating " + nImmigrants[x] + " individuals from mailbox for subpopulation " + x);
			}
			...
			if (nImmigrants[x] >= state.population.subpops[x].individuals.length)
				state.output.fatal("Number of immigrants (" + nImmigrants[x] + ") is larger than subpopulation #" + x + "'s size (" + len
