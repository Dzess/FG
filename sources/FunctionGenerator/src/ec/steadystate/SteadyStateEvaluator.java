...
/**
 * This subclass of Evaluator performs the evaluation portion of Steady-State
 ...
 * 
 * <p>
 * When SteadyStateEvaluator sends indivduals off to be evaluated, it stores
 * them in an internal queue, along with the subpopulation in which they were
 * destined. This tuple is defined by QueueIndividual.java
 * 
 * 
 * @author Sean Luke
 * @version 1.0
...
public class SteadyStateEvaluator extends SimpleEvaluator {
	...
	/**
	 * Submits an individual to be evaluated by the Problem, and adds it and its
	 * subpopulation to the queue.
	 */
	...
	/**
	 * Returns true if we're ready to evaluate an individual. Ordinarily this is
	 * ALWAYS true, except in the asynchronous evolution situation, where we may
	 * not have a processor ready yet.
	 */
	...
	/**
	 * Returns an evaluated individual is in the queue and ready to come back to
	 * us. Ordinarily this is ALWAYS true at the point that we call it, except
	 * in the asynchronous evolution situation, where we may not have a job
	 * completed yet, in which case NULL is returned. Once an individual is
	 * returned by this function, no other individual will be returned until the
	 * system is ready to provide us with another one. NULL will be returned
	 * otherwise.
	 */
	...
	/**
	 * Returns the subpopulation of the last evaluated individual returned by
	 * getNextEvaluatedIndividual, or potentially -1 if
	 * getNextEvaluatedIndividual was never called or hasn't returned an
	 * individual yet.
	 */
