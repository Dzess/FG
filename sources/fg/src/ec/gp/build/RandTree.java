...
public class RandTree extends GPNodeBuilder {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (!canPick())
            state.output.fatal("RandTree requires some kind of size distribution set, either with " + P_MINSIZE + "/"
    ...
    // slots.
    private GPNode buildTree(final EvolutionState state, final int thread, final GPNodeParent parent,
            final int argposition, final GPFunctionSet set, final String dyckWord) {
        int counter = 0;
        Stack s = new Stack();
        char nextChar;
