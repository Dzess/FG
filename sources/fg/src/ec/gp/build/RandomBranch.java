...
/**
 * RandomBranch implements the <tt>Random_Branch</tt> tree generation method
 * described in
 * 
 * <p>
 * Chellapilla, K. 1998. Evolving Modular Programs without Crossover. in
 * <i>Proceedings of the Third Annual Genetic Programming Conference</i> (GP98),
 * J.R. Koza <i>et al</i>, editors. San Fransisco: Morgan Kaufmann. 23--31.
 * 
 * <p>
 * This algorithm attempts to create a tree of size <tt>requestedSize</tt>, or
 * "slightly less".
 * 
 * If the pipeline does not specify a size it wants (it uses
 * <tt>NOSIZEGIVEN</tt>), the algorithm picks a size at random from either
 * [minSize...maxSize] or from sizeDistribution (one of the two <b>must</b> be
 * defined), and attempts to create a tree of that size or "slightly less".
 * 
 * @author Sean Luke
 * @version 1.0
 */
...
public class RandomBranch extends GPNodeBuilder {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (!canPick())
            state.output.fatal("RandomBranch requires some kind of size distribution set, either with " + P_MINSIZE
                    + "/" + P_MAXSIZE + ", or with " + P_NUMSIZES + ".", base, defaultBase());
    }
    ...
    public GPNode newRootedTree(final EvolutionState state, final GPType type, final int thread,
        ...
        if (requestedSize < 1)
            state.output
                    .fatal("ec.gp.build.RandomBranch requested to build a tree, but a requested size was given that is < 1.");
        return randomBranch(state, type, requestedSize, thread, parent, argposition, set);
    }

    private GPNode randomBranch(final EvolutionState state, final GPType type, final int maxLength, final int thread,
            final GPNodeParent parent, final int argposition, final GPFunctionSet set) {
        // randomBranch can mess up if there are no available terminals for a
        // given type. If this occurs,
        // and we find ourselves unable to pick a terminal when we want to do
        // so, we will issue a warning,
        // and pick a nonterminal, violating the maximum-size contract. This can
                ...
                terminals.length != 0) // AND if there are available terminals
        {
            ...
            } else // we've got nonterminals, pick one at random
            {
                ...
                for (int x = 0; x < childtypes.length; x++)
                    n.children[x] = randomBranch(state, childtypes[x], (maxLength - 1) / childtypes.length, // note
                                                                                                            // int
                                                                                                            // division
                            thread, n, x, set);
                return n;
