 ...
 * 
 * <p>
 * MutatePromotePipeline tries <i>tries</i> times to find a tree that has at
 * least one promotable node. It then picks randomly from all the promotable
 * nodes in the tree, and promotes one. If it cannot find a valid tree in
 * <i>tries</i> times, it gives up and simply copies the individual.
 * 
 * <p>
 * "Promotion" means to take a node <i>n</i> whose parent is <i>m</i>, and
 * replacing the subtree rooted at <i>m</i> with the subtree rooted at <i>n</i>.
 * 
 * <p>
 * A "Promotable" node means a node which is capable of promotion given the
 * existing type constraints. In general to promote a node <i>foo</i>,
...
public class MutatePromotePipeline extends GPBreedingPipeline {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (numTries == 0)
			state.output.fatal("MutatePromotePipeline has an invalid number of tries (it must be >= 1).", base.push(P_NUM_TRIES),
	...
	private boolean promotable(final GPInitializer initializer, final GPNode node) {
		...
		// 2: the node's returntype is type-compatible with its GRANDparent's
		// return slot
	...
	private int numPromotableNodes(final GPInitializer initializer, final GPNode root, int soFar) {
	...
	// sticks the node in
	private int pickPromotableNode(final GPInitializer initializer, final GPNode root, int num) {
		if (promotable(initializer, root)) {
			num--;
			if (num == -1) // found it
			{
	...
	public int produce(final int min, final int max, final int start, final int subpopulation, final Individual[] inds,
		...
		for (int q = start; q < n + start; q++) {
			...
			if (tree != TREE_UNFIXED && (tree < 0 || tree >= i.trees.length))
				// uh oh
				state.output
						.fatal("MutatePromotePipeline attempted to fix tree.0 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual");
			...
			for (int x = 0; x < numTries; x++) {
				...
				// promote the node, or if we're unsuccessful, just leave it
				// alone
