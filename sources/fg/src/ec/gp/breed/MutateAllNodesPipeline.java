...
public class MutateAllNodesPipeline extends GPBreedingPipeline {
    ...
    private GPNode pickCompatibleNode(final GPNode original, final GPFunctionSet set, final EvolutionState state,
        ...
        if (initializer.numAtomicTypes + initializer.numSetTypes == 1) // easy
                                                                         ...
                                                                         // len
            {
                ...
                for (int y = 0; y < set.nodesByArity[type][len][x].constraints(initializer).childtypes.length; y++)
                    if (!set.nodesByArity[type][len][x].constraints(initializer).childtypes[y].compatibleWith(
                            initializer, original.children[y].constraints(initializer).returntype)) {
                        failed = true;
                        break;
                    }
                if (!failed)
    ...
    private GPNode generateCompatibleTree(final GPNode original, final GPFunctionSet set, final EvolutionState state,
        ...
        for (int x = 0; x < node.children.length; x++) {
            node.children[x] = generateCompatibleTree(original.children[x], set, state,
                    original.constraints(initializer).childtypes[x], thread);
