...
/**
 * <p>
 * GESpecies generates GPIndividuals from GEIndividuals through the application
 * of a grammar parse graph computed by the GrammarParser.
 * 
 * <p>
 * GESpecies uses a <b>GrammarParser</b> to do its dirty work. This parser's job
 * is to take a grammar (in the form of a BufferedReader) and convert it to a
 * tree of GrammarNodes which define the parse graph of the grammar. The
 * GESpecies then interprets his parse graph according to the values in the
 * GEIndividual to produce the equivalent GPIndividual, which is then evaluated.
 * 
 * <p>
 * To do this, GESpecies relies on a subsidiary GPSpecies which defines the
 * GPIndividual and various GPFunctionSets from which to build the parser. This
 * is a grand hack -- the GPSpecies does not know it's being used this way, and
 * so we must provide various dummy parameters to keep the GPSpecies happy even
 * though they'll never be used.
 * 
 * <p>
 * If you are daring, you can replace the GrammarParser with one of your own to
 * customize the parse structure and grammar.
 * 
 * <p>
 * <b>ECJ's Default GE Grammar</b> GE traditionally can use any grammar, and
 * builds parse graphs from that. For simplicity, and in order to remain as
 * compatable as possible with ECJ's existing GP facilities (and GP tradition),
 * ECJ only uses a single Lisp-like grammar which generates standard ECJ trees.
 * This doesn't lose much in generality as the grammar is quite genral.
 * 
 * <p>
 * The grammar assumes that expansion points are enclosed in &lt;> and functions
 ...
 * 
 * <p>
 * Note that you can use several lines to define the same grammar rule: for
 * example, <tt>&lt;op></tt> was defined by several lines when it could have
 * consisted of several elements separated by vertical pipes ( <tt>|</tt> ).
 * Either way is fine, or a combination of both.
 * 
 * <p>
 * GPNodes are included in the grammar by using their name. This includes ERCs,
 * ADFs, ADMs, and ADFArguments, which should all work just fine. For example,
 * since most ERC GPNodes are simply named "ERC", if you have only one ERC
 * GPNode in your function set, you can just use <tt>(ERC)</tt> in your grammar.
 * 
 * <p>
 * Once the gammar file has been created and setup has been run trees can the be
 * created using the genome (chromosome) of a GEIndividual. A genome of an
 * individual is an array of random integers each of which are one byte long.
 * These numbers are used when a decision point (a rule having more that one
 * choice) is reached within the grammar. Once a particular gene (index) in the
 * genome has been used it will not be used again (this may change) when
 * creating the tree.
 * 
 * <p>
 * For example:<br>
 ...
 * %lt;op> can map into one of the following: (if-food-ahead &lt;op> &lt;op>) |
 * (progn2 &lt;op> &lt;op>) | (progn3 &lt;op> &lt;op> &lt;op>) | (left) |
 * (right) | (move)<br>
 * Since the rule &lt;op> has more than one choice that it can map to, we must
 * consult the genome to decide which choice to take. In this case the number of
 * chromosomes used is 0 so genome[0] is used and number of chromosomes used is
 * incremented. Since values in the genome can be negitive values they are
 * offset by 128 (max negitive of a byte) giving us a value from 0-255. A
 * modulus is performed on this resulting number by the number of choices
 * present for the given rule. In the above example since we are using genome[0]
 * the resulting operation would look like: 23+128=151, number of choices for
 * &lt;op> = 6, 151%6=1 so we use choices[1] which is: (progn2 &lt;op> &lt;op>).
 * If all the genes in a genome are used and the tree is still incompete an
 * invalid tree error is returned.
 * 
 * <p>
 ...
 * <td valign=top>(the file is where the rules of the gammar are stored)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>gp-species</tt><br>
 * <font size=-1>classname, inherits and != ec.gp.GPSpecies</font></td>
 * <td valign=top>(the GPSpecies subservient to the GESpecies)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>parser</tt><br>
 * <font size=-1>classname, inherits and != ec.gp.ge.GrammarParser</font></td>
 * <td valign=top>(the GrammarParser used by the GESpecies)</td>
 * </tr>
 * 
 * </table>
 * 
...
public class GESpecies extends IntegerVectorSpecies {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (!(i_prototype instanceof ByteVectorIndividual)) {
            state.output.fatal("The Individual class for the Species " + getClass().getName()
                    + " is must be a subclass of ec.gp.ge.GEIndividual.", base);
        }
        ...
        parser_prototype = (GrammarParser) (state.parameters.getInstanceForParameterEq(base.push(P_PARSER),
                def.push(P_PARSER), GrammarParser.class));
        ...
        for (int i = 0; i < numGrammars; i++) {
            ...
            if (grammarFile == null) {
                state.output.fatal("Error retrieving grammar file(s): " + def.toString() + "." + P_FILE + "." + i
                        + " is undefined.");
            }
     ...
     *            Evolution state
     * @param trees
     *            array of trees for the individual
     * @param ind
     *            the GEIndividual
     * @param threadnum
     *            tread number
    ...
    /**
     * makeTree, edits the tree that its given by adding a root (and all
     * subtrees attached)
     * 
     * @param state
     * @param ind
     ...
     * @param treeNum
     * @param threadnum
     * @return the number of chromosomes used, or an BIG_TREE_ERROR sentinel
     *         value.
     */
    public int makeTree(EvolutionState state, GEIndividual ind, GPTree tree, int position, int treeNum, int threadnum) {
        int[] countNumberOfChromosomesUsed = { position }; // hack, use an array
                                                           // to pass an extra
                                                           // value
    ...
    // thrown by makeSubtree when chromosome is not large enough for the
    // generated tree.
    class BigTreeException extends RuntimeException {
        static final long serialVersionUID = 1L;
    }

    GPNode makeSubtree(int[] index, byte[] genome, EvolutionState es, GPFunctionSet gpfs, GrammarRuleNode rule,
            int treeNum, int threadnum) {
        ...
        // more than one rule to consider, pick one based off the genome, and
        // consume the current gene
        if (rule.getNumChoices() > 1) {
            // casting to an int should be ok since the biggest these genes can
            // be is a byte
        ...
        } else // handle functions
        {
            ...
            // does the grammar contain the correct amount of children that the
            // GPNode requires
    ...
    /**
     * Loads an ERC from the ERCBank given the value in the genome. If there is
     * no such ERC, then one is created and randomized, then added to the bank.
     * The point of this mechanism is to enable ERCs to appear in multiple
     * places in a GPTree.
     */
    ...
    /**
     * Returns the number of elements consumed from the GEIndividual array to
     * produce the tree, else returns -1 if an error occurs, specifically if all
     * elements were consumed and the tree had still not been completed.
     */
    ...
    /**
     * Returns a dummy GPIndividual with a single tree which was built by
     * mapping over the elements of the given GEIndividual. Null is returned if
     * an error occurs, specifically, if all elements were consumed and the tree
