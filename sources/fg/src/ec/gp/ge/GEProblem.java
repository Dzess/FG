...
/**
 * GEProblem is a special replacement for Problem which performs GE mapping. You
 * do not subclass from GEProblem. Rather, create a GPProblem subclass and set
 * it to be the 'problem' parameter of the GEProblem. The GEProblem will convert
 * the GEIndividual into a GPIndividual, then pass this GPIndividual to the
 * GPProblem to be evaluated.
 * 
 * <p>
 * The procedure is as follows. Let's say your GPProblem is the Artificial Ant
 * problem. Instead of saying...
 * 
 * <p>
 * <tt>eval.problem = ec.app.ant.Ant<br>
 ...
 * 
 * <p>
 * <tt>eval.problem = ec.gp.ge.GEProblem</tt>
 * 
 * <p>
 * ... and then you hang the Ant problem, and all its subsidiary data, as the
 * 'problem' parameter from the GEProblem like so:
 * 
 * <p>
 * <tt>eval.problem.problem = ec.app.ant.Ant<br>
 ...
 * 
 * <p>
 * Everything else should be handled for you. GEProblem is also compatible with
 * the MasterProblem procedure for distributed evaluation, and is also both a
 * SimpleProblemForm and a GroupedProblemForm. We've got you covered.
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>problem</tt><br>
 * <font size=-1>classname, inherits from GPProblem</font></td>
 * <td valign=top>(The GPProblem which actually performs the evaluation of the
 * mapped GPIndividual)</td>
 * </tr>
 * </table>
 */
...
public class GEProblem extends Problem implements SimpleProblemForm, GroupedProblemForm {
	...
	/**
	 * Default version assumes that every individual is a GEIndividual. The
	 * underlying problem.evaluate() must be prepared for the possibility that
	 * some GPIndividuals handed it are in fact null, meaning that they couldn't
	 * be extracted from the GEIndividual string. You should assign them bad
	 * fitness in some appropriate way.
	 */
	public void evaluate(final EvolutionState state, final Individual[] ind, // the
		...
		// the default version assumes that every subpopulation is a GE
		// Individual
		...
		for (int i = 0; i < gpi.length; i++) {
			...
			// the GPIndividual's fitness because even though the mapping
			// function
