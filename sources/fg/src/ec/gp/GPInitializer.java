	...
	public void setupNodeConstraints(final EvolutionState state, final Parameter base) {
		...
		for (int y = 0; y < x; y++) {
			...
			if (state.parameters.exists(base.push("" + y), null))
				...
				state.output.message("No GP Node Constraints specified, assuming the default class: ec.gp.GPNodeConstraints for "
						+ base.push("" + y));
	...
	public void setupFunctionSets(final EvolutionState state, final Parameter base) {
		...
		for (int y = 0; y < x; y++) {
			...
			if (state.parameters.exists(base.push("" + y), null))
				...
				state.output
						.message("No GPFunctionSet specified, assuming the default class: ec.gp.GPFunctionSet for " + base.push("" + y));
				c = new GPFunctionSet();
			}
			c.setup(state, base.push("" + y));
		}
	...
	public void setupTreeConstraints(final EvolutionState state, final Parameter base) {
		...
		for (int y = 0; y < x; y++) {
			...
			if (state.parameters.exists(base.push("" + y), null))
				...
				state.output.message("No GP Tree Constraints specified, assuming the default class: ec.gp.GPTreeConstraints for "
						+ base.push("" + y));
