 ...
 * read back in 100% by ECJ as well. Because GPIndividuals are often very large,
 * <b>GPIndividual has overridden these methods -- they work differently than in
 * Individual (the superclass).</b> In specific: <b>readIndividual</b> by
 * default reads in the fitness and the evaluation flag, then calls
 * <b>parseGenotype</b> to read in the trees (via GPTree.readTree(...)). However
 * <b>printIndividual</b> by default prints the fitness and evaluation flag, and
 * prints all the trees by calling GPTree.printTree(...). It does not call
 * <b>genotypeToString</b> at all. This is because it's very wasteful to build
 * up a large string holding the printed form of the GPIndividual just to pump
 * it out a stream once.
 * 
...
public class GPIndividual extends Individual {
    ...
    /**
     * Sets up a prototypical GPIndividual with those features which it shares
     * with other GPIndividuals in its species, and nothing more.
     */
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        // how many trees?
        int t = state.parameters.getInt(base.push(P_NUMTREES), def.push(P_NUMTREES), 1); // at
                                                                                         // least
                                                                                         // 1
                                                                                         // tree
                                                                                         // for
    ...
    /**
     * Prints just the trees of the GPIndividual. Broken out like this to be
     * used by GEIndividual to avoid re-printing the fitness and evaluated
     * premables.
     */
    ...
    public void readGenotype(final EvolutionState state, final DataInput dataInput) throws IOException {
        ...
        if (trees == null || treelength != trees.length) // wrong size!
            state.output
                    .fatal("Number of trees differ in GPIndividual when reading from readGenotype(EvolutionState, DataInput).");
        for (int x = 0; x < trees.length; x++)
     ...
     * with null roots, and this method, which calls GPTree.clone(), will
     * produce a NullPointerException as a result. Instead, you probably want to
     * use GPSpecies.newIndividual(...) if you're thinking of playing with the
    ...
    public GPIndividual lightClone() {
        ...
        myobj.trees = new GPTree[trees.length];
        for (int x = 0; x < trees.length; x++) {
            myobj.trees[x] = (GPTree) (trees[x].lightClone()); // note
                                                               // light-cloned!
            myobj.trees[x].owner = myobj; // reset owner away from me
        }
        return myobj;
    ...
    /**
     * Returns the "size" of the individual, namely, the number of nodes in all
     * of its subtrees.
     */
    public long size() {
        long size = 0;
