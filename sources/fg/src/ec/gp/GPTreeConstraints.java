 ...
 * to many different GPTree trees, namely the tree type, builder, and function
 * set. GPTreeConstraints have unique names by which they are identified.
 * 
 * <p>
 * In adding new things to GPTreeConstraints, you should ask yourself the
 * following questions: first, is this something that takes up too much memory
 * to store in GPTrees themseves? second, is this something that needs to be
 * accessed very rapidly, so cannot be implemented as a method call in a GPTree?
 ...
 * <td valign=top>(number of tree constraints)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.n</i>.<tt>name</tt><br>
 * <font size=-1>String</font></td>
 * <td valign=top>(name of tree constraint <i>n</i>)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.n</i>.<tt>init</tt><br>
 * <font size=-1>classname, inherits and != ec.gp.GPNodeBuilder</font></td>
 * <td valign=top>(GP node builder for tree constraint <i>n</i>)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.n</i>.<tt>returns</tt><br>
 * <font size=-1>String</font></td>
 * <td valign=top>(tree type for tree constraint <i>n</i>)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.n</i>.<tt>fset</tt><br>
 * <font size=-1>String</font></td>
 * <td valign=top>(function set for tree constraint <i>n</i>)</td>
 * </tr>
 * 
 * </table>
 * 
...
public class GPTreeConstraints implements Clique {
    ...
    /**
     * This must be called <i>after</i> the GPTypes and GPFunctionSets have been
     * set up.
     */
    public final void setup(final EvolutionState state, final Parameter base) {
        ...
        // Register me
        GPTreeConstraints old_constraints = (GPTreeConstraints) (((GPInitializer) state.initializer).treeConstraintRepository
                .put(name, this));
        if (old_constraints != null)
            state.output.fatal("The GP tree constraint \"" + name + "\" has been defined multiple times.",
                    base.push(P_NAME));
        ...
        state.output.exitIfErrors(); // otherwise checkFunctionSetValidity might
                                     // crash below
        ...
        while (e.hasMoreElements()) {
            ...
            if (i.length == 0) // yeesh
                state.output
                        .error("In function set "
                                + functionset
                                + " for the GPTreeConstraints "
                                + this
                                + ", no nodes at all are given with the return type "
                                + t
                                + " which is required by other functions in the function set or by the tree's return type.  This almost certainly indicates a serious typing error.",
                                base);
            else {
                    ...
                    state.output
                            .warning(
                                    "In function set "
                                            ...
                                            + " for the GPTreeConstraints "
                                            + this
                                            + ", no *nonterminals* are given with the return type "
                                            + t
                                            + " which is required by other functions in the function set or by the tree's return type.  This may or may not be a problem for you.",
                                    base);
            }
        }
    ...
    private void checkFunctionSetValidity(final EvolutionState state, final Hashtable done, final GPType type) {
    ...
    public static GPTreeConstraints constraintsFor(final String constraintsName, final EvolutionState state) {
        GPTreeConstraints myConstraints = (GPTreeConstraints) (((GPInitializer) state.initializer).treeConstraintRepository
                .get(constraintsName));
