...
/**
 * A GPSetType is a GPType which contains GPAtomicTypes in a set, and is used as
 * a generic GP type. For more information, see GPType
 * 
 * GPSetTypes implement their set using both a hash table and an array. if the
 * size of the set is "significantly big", then the hash table is used to look
 * up membership in the set (O(1), but with a big constant). If the size is
 * small, then the array is used (O(n)). The dividing line is determined by the
 * constant START_USING_HASH_BEYOND, which you might play with to optimize for
 * your system.
 * 
 * @see ec.gp.GPType
 * @author Sean Luke
...
public final class GPSetType extends GPType {
    ...
    public void setup(final EvolutionState state, Parameter base) {
        ...
        if (len <= 0)
            state.output.fatal("The number of atomic types in the GPSetType " + name + " must be >= 1.",
                    base.push(P_SIZE));
        ...
        for (int x = 0; x < len; x++) {
            ...
            if (s == null)
                state.output.fatal("Atomic type member #" + x + " is not defined for the GPSetType " + name + ".", base
                        .push(P_MEMBER).push("" + x));
            GPType t = GPType.typeFor(s, state);
            if (!(t instanceof GPAtomicType)) // uh oh
                state.output.fatal("Atomic type member #" + x + " of GPSetType " + name + " is not a GPAtomicType.",
                        base.push(P_MEMBER).push("" + x));

            if (types_h.get(t) != null)
                state.output.warning("Atomic type member #" + x + " is included more than once in GPSetType " + name
                        + ".", base.push(P_MEMBER).push("" + x));
            types_h.put(t, t);
        }
    }
    ...
    public final boolean compatibleWith(final GPInitializer initializer, final GPType t) {
        ...
        // if the type is an atomic type, then I'm compatible with it if I
        // contain it.
        // Use the sparse array.
        else if (t.type < initializer.numAtomicTypes) // atomic type, faster
                                                      // than doing instanceof
