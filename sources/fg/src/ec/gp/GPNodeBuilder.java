...
public abstract class GPNodeBuilder implements Prototype {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (state.parameters.exists(base.push(P_MINSIZE), def.push(P_MINSIZE))) {
			...
			state.output.fatal("This GPNodeBuilder has a " + P_MAXSIZE + " but not a " + P_MINSIZE + ".", base.push(P_MAXSIZE),
					def.push(P_MAXSIZE));
		...
		// load sizeDistribution
			...
			if (state.parameters.exists(base.push(P_SIZE).push("0"), def.push(P_SIZE).push("0")))
				state.output.warning("GPNodeBuilder does not use size #0 in the distribution", base.push(P_SIZE).push("0"), def
						.push(P_SIZE).push("0"));

			float sum = 0.0f;
			for (int x = 0; x < siz; x++) {
				sizeDistribution[x] = state.parameters.getFloat(base.push(P_SIZE).push("" + (x + 1)), def.push(P_SIZE).push("" + (x + 1)),
						0.0f);
				if (sizeDistribution[x] < 0.0) {
					state.output.warning("Distribution value #" + x + " negative or not defined, assumed to be 0.0", base.push(P_SIZE)
							.push("" + (x + 1)), def.push(P_SIZE).push("" + (x + 1)));
	...
	public abstract GPNode newRootedTree(final EvolutionState state, final GPType type, final int thread, final GPNodeParent parent,
	...
	protected void warnAboutNonTerminalWithType(GPType type, boolean fail, EvolutionState state) {
		...
		state.output.warnOnce("A GPNodeBuilder has been requested at least once to generate a tree with "
				...
				+ "in this way.  As a result, the algorithm was forced to use a TERMINAL, making the tree smaller than " + "requested.");
