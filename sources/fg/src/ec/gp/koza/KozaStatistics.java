 ...
 * (raw,adjusted,hits) and best individual of each generation. At the end,
 * prints the best individual of the run and the number of individuals
 * processed.
 * 
 * <p>
 * If gather-full is true, then final timing information, number of nodes and
 * depths of trees, approximate final memory utilization, etc. are also given.
 * 
 * <p>
 ...
 * 
 * <p>
 * KozaStatistics implements a simple version of steady-state statistics in the
 * same fashion that SimpleStatistics does: if it quits before a generation
 * boundary, it will include the best individual discovered, even if the
 * individual was discovered after the last boundary. This is done by using
 * individualsEvaluatedStatistics(...) to update best-individual-of-generation
 ...
 * </tr>
 * <tr>
 * <td valign=top><i>base</i>.<tt>gather-full</tt><br>
 * <font size=-1>bool = <tt>true</tt> or <tt>false</tt> (default)</font></td>
 * <td valign=top>(should we full statistics on individuals (will run slower,
 * though the slowness is due to off-line processing that won't mess up timings)
 * </td>
 * </tr>
 * </table>
 * 
...
public class KozaStatistics extends Statistics implements SteadyStateStatisticsForm {
    ...
    public void postEvaluationStatistics(final EvolutionState state) {
        ...
        for (int x = 0; x < state.population.subpops.length; x++) {
            ...
            state.output.print("Mean fitness raw: " + meanStandardized + " adjusted: " + meanAdjusted + " hits: "
                    + ((double) hits) / state.population.subpops[x].individuals.length, statisticslog);
    ...
    public void finalStatistics(final EvolutionState state, final int result) {
        ...
        if (doFull) {
            ...
            state.output.println("Evaluating: " + ((float) evaluationTime) / 1000 + " secs total, " + nodesEvaluated
                    + " nodes, " + nodesEvaluated / (((float) evaluationTime) / 1000) + " nodes/sec", statisticslog);
            state.output.println("Breeding: " + ((float) breedingTime) / 1000 + " secs total, " + nodesBred
                    + " nodes, " + nodesBred / (((float) breedingTime) / 1000) + " nodes/sec", statisticslog);

            state.output.println("\n\n\nMemory Usage\n==============", statisticslog);
            state.output.println("Initialization: " + ((float) initializationUsage) / 1024 + " KB total, "
                    + nodesInitialized + " nodes, " + nodesInitialized / (((float) initializationUsage) / 1024)
                    + " nodes/KB", statisticslog);
            state.output.println("Evaluating: " + ((float) evaluationUsage) / 1024 + " KB total, " + nodesEvaluated
                    + " nodes, " + nodesEvaluated / (((float) evaluationUsage) / 1024) + " nodes/KB", statisticslog);
