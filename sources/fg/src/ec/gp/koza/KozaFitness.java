 ...
 * are considered the same (there are different methods for them, but they
 * return the same thing). Standardized fitness ranges from 0.0 inclusive (the
 * best) to infinity exclusive (the worst). Adjusted fitness converts this,
 * using the formula adj_f = 1/(1+f), into a scale from 0.0 exclusive (worst) to
 * 1.0 inclusive (best). While it's the standardized fitness that is stored, it
 * is the adjusted fitness that is printed out. This is all just convenience
 * stuff anyway; selection methods generally don't use these fitness values but
 * instead use the betterThan and equalTo methods.
 * 
 * <p>
 * <b>Default Base</b><br>
 * gp.koza.fitness
...
public class KozaFitness extends Fitness {
    ...
    /**
     * This ranges from 0 (best) to infinity (worst). I define it here as
     * equivalent to the standardized fitness.
     */
    protected float standardizedFitness;

    /**
     * This auxillary measure is used in some problems for additional
     * information. It's a traditional feature of Koza-style GP, and so although
     * I think it's not very useful, I'll leave it in anyway.
     */
    public int hits;
    ...
    /**
     * Do not use this function. Use the identical setStandardizedFitness()
     * instead. The reason for the name change is that fitness() returns a
     * differently-defined value than setFitness() sets, ugh.
     * 
     * @deprecated
     */
     ...
     * which is defined (NOTE: DIFFERENT FROM fitness()!!!) as 0.0 being the
     * IDEAL and infinity being worse than the worst possible. This is the GP
     * tradition. The fitness() function instead will output the equivalent of
     * Adjusted Fitness.
     */
    ...
    /**
     * Returns the adjusted fitness metric, which recasts the fitness to the
     * half-open interval (0,1], where 1 is ideal and 0 is worst. Same as
     * adjustedFitness().
     */
    ...
    /**
     * Returns the adjusted fitness metric, which recasts the fitness to the
     * half-open interval (0,1], where 1 is ideal and 0 is worst. This metric is
     * used when printing the fitness out.
     */
    ...
    public String fitnessToStringForHumans() {
        return FITNESS_PREAMBLE + "Standardized=" + standardizedFitness + " Adjusted=" + adjustedFitness() + " Hits="
                + hits;
    }

    public final void readFitness(final EvolutionState state, final LineNumberReader reader) throws IOException {
