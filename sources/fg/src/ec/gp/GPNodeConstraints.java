...
/**
 * A GPNodeConstraints is a Clique which defines constraint information common
 * to many different GPNode functions, namely return types, child types, and
 * number of children. GPNodeConstraints have unique names by which they are
 * identified.
 * 
 * <p>
 * In adding new things to GPNodeConstraints, you should ask yourself the
 * following questions: first, is this something that takes up too much memory
 * to store in GPNodes themselves? second, is this something that needs to be
 * accessed very rapidly, so cannot be implemented as a method call in a GPNode?
 * third, can this be shared among different GPNodes, even ones representing
 * different functions?
 * 
 * <p>
 * <b>Parameters</b><br>
 ...
 * <td valign=top>(number of node constraints)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.n</i>.<tt>name</tt><br>
 * <font size=-1>String</font></td>
 * <td valign=top>(name of node constraint <i>n</i>)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.n</i>.<tt>returns</tt><br>
 * <font size=-1>String</font></td>
 * <td valign=top>(return type for node constraint <i>n</i>)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.n</i>.<tt>size</tt><br>
 * <font size=-1>int &gt;= 1</font></td>
 * <td valign=top>(number of child arguments for node constraint <i>n</i>)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.n</i>.<tt>child.</tt><i>m</i><br>
 * <font size=-1>String</font></td>
 * <td valign=top>(name of type for child argument <i>m</i> of node constraint
 * <i>n</i>)</td>
 * </tr>
 * 
 * <tr>
 ...
 * <td valign=top>(auxillary probability of selection -- used by
 * ec.gp.build.PTC1 and ec.gp.build.PTC2)</td>
 * </tr>
 * 
 * </table>
 * 
...
public class GPNodeConstraints implements Clique {
    ...
    /**
     * The name of the GPNodeConstraints object -- this is NOT the name of the
     * GPNode
     */
    public String name;
    ...
    /**
     * A little memory optimization: if GPNodes have no children, they are
     * welcome to use share this zero-sized array as their children array.
     */
    ...
    public final void setup(final EvolutionState state, final Parameter base) {
        ...
        // Register me
        GPNodeConstraints old_constraints = (GPNodeConstraints) (((GPInitializer) state.initializer).nodeConstraintRepository
                .put(name, this));
        if (old_constraints != null)
            state.output.fatal("The GP node constraint \"" + name + "\" has been defined multiple times.",
                    base.push(P_NAME));
        ...
        if (state.parameters.exists(base.push(P_PROBABILITY), null)) {
            ...
            if (f < 0)
                state.output.fatal("The probability of selection is < 0, which is not valid.",
                        base.push(P_PROBABILITY), null);
        ...
        if (x < 0)
            state.output.fatal("The number of children types for the GPNodeConstraints " + name + " must be >= 0.",
                    base.push(P_SIZE));
        ...
        for (x = 0; x < childtypes.length; x++) {
            ...
            if (s == null)
                state.output.fatal("Type #" + x + " is not defined for the GPNodeConstraints " + name + ".",
                        base.push("" + x));
    ...
    public static GPNodeConstraints constraintsFor(final String constraintsName, final EvolutionState state) {
        GPNodeConstraints myConstraints = (GPNodeConstraints) (((GPInitializer) state.initializer).nodeConstraintRepository
                .get(constraintsName));
