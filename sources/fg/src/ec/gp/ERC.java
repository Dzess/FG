...
/**
 * ERC is an abstract GPNode which implements Ephemeral Random Constants, as
 * described in Koza I. An ERC is a node which, when first instantiated, gets
 * set to some random constant value which it always returns from then on, even
 * after being crossed over into other individuals. In order to implement an
 * ERC, you need to override several methods below.
 * 
 ...
 * properly. If your ERC data is just a simple or immutable type (like an int or
 * a string), you don't need write a clone() method; the default one works fine.
 * But if your data is an array or other mutable object, you'll need to override
 * the clone() method to copy the array.
 * 
 * <li>An implementation of the <b>resetNode</b> method to randomize the data
 * once cloned from the prototype. This essentially "initializes" your ERC.
 * 
 * <li>An implementation of the <b>encode</b> method which presents the ERC as a
 * String. If you don't plan on writing individuals out to files in a fashion
 * that enables them to be read back in again later, but only care to print out
 * individuals for statistics purposes, you can implement this to just write
 * <tt>"" + <i>value</i></tt>, where <i>value</i> is your data.
 * 
 * <li>An implementation of the <b>nodeEquals</b> method to return true if the
 * other node is also an ERC of the same type, and it has the same ERC data as
 * yourself.
 * 
 * 
 * </ul>
 ...
 * <ul>
 * 
 * <li>If you have ERCs of different class types (for example, a vector ERC and
 * a floating-point scalar ERC), you will wish to distinguish them when they're
 * printed to files. To do this, override the <b>name</b> method to return
 * different strings for each of them (perhaps "vec" versus "").
 * 
 * <li>If you want to write your ERCs to files such that they can be read back
 * in again, you'll need to override the <b>encode</b> method to write using the
 * <tt>ec.util.Code</tt> class. Further, you'll need to override the
 * <b>decode</b> method to read in the individual using the
 * <tt>ec.util.Code</tt> and <tt>ec.util.DecodeReturn</tt> classes. The default
 * version -- which is wrong -- returns <tt>false</tt>. When you do this, you'll
 * probably also want to override the <b>toStringForHumans()</b> method to
 * return a simple string form of the ERC: perhaps just a number or a vector
 * like "<7.24, 9.23>". This is because by default <b>toStringForHumans()</b>
 ...
 * <b>ec.gp.breed.MutateERCPipeline</b>. If you expect to mutate your ERCs, you
 * may wish to override the <b>mutateERC</b> method to do something more subtle
 * than its default setting (which just randomizes the ERC again, by calling
 * resetNode).
 * 
 * <li>The default <b>nodeHashCode</b> implementation is poor and slow (it
 * creates a string using encode() and then hashes the sting). You might create
 * a better (and probably simpler) hash code function.
 * 
 * <li>If you're going to use facilities such as the Island Model or the
 * distributed evaluator, you'll need to implement the <b>writeNode</b> and
 * <b>readNode</b> methods to read/write the node to DataInput/DataOutput. The
 * default implementations just throw errors.
 * 
 * <li>If you need to set up your ERC class from the parameter file, do so in
 * the <b>setup</b> method.
 * 
 * </ul>
 * 
 * <p>
 * See the <b>ec.app.regression.func.RegERC</b> class for an example of a simple
 * but "fuly-implemented" ERC. A slightly more complicated example can be found
 * in <b>ec.app.lawnmower.func.LawnERC</b>.
 * 
 * @author Sean Luke
 * @version 1.0
 */
...
public abstract class ERC extends GPNode {
    ...
    public void checkConstraints(final EvolutionState state, final int tree, final GPIndividual typicalIndividual,
        ...
        // make sure we don't have any children. This is the typical situation
        // for an ERC.
