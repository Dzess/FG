		...
		try {
			if (obj == null || type.isPrimitive() || index < 0)
				...
				return new ReflectedObject(
						f[index].get(obj),
						(f[index].get(obj) == null || f[index].getType().isPrimitive() ? f[index].getType() : f[index].get(obj).getClass()),
	...
	public ReflectedObject[] getFields() {
		try {
			if (obj == null || type.isPrimitive())
				...
				ReflectedObject[] ref = new ReflectedObject[len];
				for (int x = 0; x < len; x++)
					ref[x] = new ReflectedObject(f[x].get(obj), (f[x].get(obj) == null || f[x].getType().isPrimitive() ? f[x].getType()
							: f[x].get(obj).getClass()), f[x].getName(), f[x]);
				return ref;
			}
		} catch (IllegalArgumentException e) {
			e.printStackTrace();
		...
		try {
			...
			for (int x = 0; x < m.length; x++) {
																							...
																							// syntax?
				{
					...
					if (m[x].getParameterTypes().length == 0 && Modifier.isPublic(modifier) && m[x].getReturnType() != Void.TYPE) // no
																																	// arguments,
																																	// and
																																	// public,
																																	// non-void,
																							...
																							// syntax?
				{
																																	...
																																	// non-abstract?
					{
						if (count == index) {
							...
							return new ReflectedObject(o, o == null || m[x].getReturnType().isPrimitive() ? m[x].getReturnType()
			...
			for (int x = 0; x < m.length; x++) {
																							...
																							// syntax?
				{
																																	...
																																	// non-abstract?
					{
						...
						try {
							o = m[x].invoke(obj, new Object[0]);
							refs[count] = new ReflectedObject(o, o == null || m[x].getReturnType().isPrimitive() ? m[x].getReturnType()
									: o.getClass(), "Property " + m[x].getName(), "Property " + m[x].getName());
						} catch (InvocationTargetException e) {
							refs[count] = unknown;
						}
