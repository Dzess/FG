...
public class ReflectedObject implements TreeModel {
    ...
    Object obj; // the object. Primitive objects are cast into their wrapper
                // types
    String name; // the name of the instance variable holding the object.
                 // Top-level objects have a name of "->"
    Object uniq; // a unique identifier for this particular instance variable
                 // (used to make getIndexOfChild() possible, ick)
    ...
    public String toString() {
        ...
        try {
            if (obj == null)
                ...
                return field + type.getName() + ", length=" + Array.getLength(obj);
    ...
    public ReflectedObject getField(int index) {
        try {
            if (obj == null || type.isPrimitive() || index < 0)
                return null;
            else if (type.isArray()) {
                ...
                if (index > len)
                    return null;
                return new ReflectedObject(f[index].get(obj), (f[index].get(obj) == null
                        || f[index].getType().isPrimitive() ? f[index].getType() : f[index].get(obj).getClass()),
            ...
            if (obj == null || type.isPrimitive())
                ...
                for (int x = 0; x < len; x++)
                    ref[x] = new ReflectedObject(f[x].get(obj),
                            (f[x].get(obj) == null || f[x].getType().isPrimitive() ? f[x].getType() : f[x].get(obj)
                                    .getClass()), f[x].getName(), f[x]);
                return ref;
            }
        } catch (IllegalArgumentException e) {
            e.printStackTrace();
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof ReflectedObject))
            return false;
        if (uniq == null && ((ReflectedObject) obj).uniq == null)
            return true;
        if (uniq == null || ((ReflectedObject) obj).uniq == null)
            return false;
    ...
    public int getIndexOfChild(Object parent, Object child) {
        ...
        for (int x = 0; x < children.length; x++)
            if (children[x].equals(child))
                return x;
        throw new IndexOutOfBoundsException("No such child " + child + " in parent "
                + ((ReflectedObject) parent).toString());
    }
    ...
    int getNumProperties() {
        ...
        try {
            ...
            for (int x = 0; x < m.length; x++) {
                if (m[x].getName().startsWith("get") || m[x].getName().startsWith("is")) // corrrect
                                                                                         // syntax?
                {
                    int modifier = m[x].getModifiers();
                    if (m[x].getParameterTypes().length == 0 && Modifier.isPublic(modifier)
                            && m[x].getReturnType() != Void.TYPE) // no
                                                                  // arguments,
                                                                  // and
                                                                  // public,
                                                                  // non-void,
                                                                  ...
                                                                  // non-abstract?
                    {
                        if (count == index) {
                            ...
                            return new ReflectedObject(o,
                                    o == null || m[x].getReturnType().isPrimitive() ? m[x].getReturnType()
                                            : o.getClass(), "Property " + m[x].getName(), "Property " + m[x].getName());
                        }
                        count++;
                    }
                }
            ...
            for (int x = 0; x < m.length; x++) {
                                                                                         ...
                                                                                         // syntax?
                {
                                                                  ...
                                                                  // non-abstract?
                    {
                        ...
                        try {
                            o = m[x].invoke(obj, new Object[0]);
                            refs[count] = new ReflectedObject(o,
                                    o == null || m[x].getReturnType().isPrimitive() ? m[x].getReturnType()
                                            : o.getClass(), "Property " + m[x].getName(), "Property " + m[x].getName());
                        } catch (InvocationTargetException e) {
                            refs[count] = unknown;
