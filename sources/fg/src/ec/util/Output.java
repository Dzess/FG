...
/**
 * <p>
 * Outputs and logs system messages, errors, and other various items printed as
 * a result of a run.
 * 
 * <p>
 * Output maintains zero or more logs, which contain Writers which write out
 * stuff. Each log has an associated verbosity; if request is made to write text
 * to a log, and the text's maximal verbosity is lower than the verbosity of the
 * log, the log will not write it. Each Output instance also has an
 * instance-level global verbosity; incoming requests to write text are
 * additionally subject to this verbosity test. Lastly, the Output class itself
 * has a global verbosity as well. This last verbosity is useful for shutting
 * down writing to all logs in the entire system in a simple way.
 * 
 * <p>
 * When the system fails for some reason and must be started back up from a
 * checkpoint, Output's log files may be overwritten. Output offers three
 * approaches here. First, Output can clear the log file and overwrite it.
 * Second, Output can append to the existing log file; because checkpoints are
 * only done occasionally, this may result in duplicate outputs to a file, so
 * keep this in mind. Third, Output can keep certain written text, typically
 * <i>announcements</i>, in memory; this text gets written out into the
 * checkpoint file, and so it is sound.
 * 
 * <p>
 ...
 * <li>Simple ERRORs. These errors set the "errors" flag to true; at the end of
 * a stream of simple errors, the system in general is expected to exit with a
 * fatal error due to the flag being set. That's the protocol anyway. On restart
 * from a checkpoint, if there were any simple errors, the system ends with a
 ...
 * <li>SYSTEM MESSAGEs. Useful system-level facts printed out for the benefit of
 * the user.
 * </ol>
 * 
 * <!--
 * <p>
 * The default verbosity values for different kinds of announcements are given
 * below:
 * 
 * <table>
 * <tr>
 * <td>0</td>
 * <td>V_VERBOSE</td>
 * <td>(totally verbose)</td>
 * </tr>
 * <tr>
 * <td>1000</td>
 * <td>V_NO_MESSAGES</td>
 * <td>(don't print messages or system messages)</td>
 * </tr>
 * <tr>
 * <td>2000</td>
 * <td>V_NO_WARNINGS</td>
 * <td>(don't print warnings, messages, or system messages)</td>
 * </tr>
 * <tr>
 * <td>3000</td>
 * <td>V_NO_GENERAL</td>
 * <td>(don't print warnings, messages, system messages, or other "general info"
 * stuff that might come along (like statistics maybe))</td>
 * </tr>
 * <tr>
 * <td>4000</td>
 * <td>V_NO_ERRORS</td>
 * <td>(don't even print errors)</td>
 * </tr>
 * <tr>
 * <td>5000</td>
 * <td>V_TOTALLY_SILENT</td>
 * <td>(be totally silent)</td>
 * </tr>
 * </table>
 * 
 * -->
 * 
 * <p>
 * Output will also store all announcements in memory by default so as to
 * reproduce them if it's restarted from a checkpoint. You can change this
 * behavior also by
 * 
 * @author Sean Luke
 * @version 1.0
...
public class Output implements Serializable {
    ...
    /**
     * The standard verbosity to use if you don't want common reporting (like
     * statistics)
     */
    ...
    /**
     * Creates a new log of minimal verbosity <i>verbosity</i> and adds it to
     * Output. This log will write to the file <i>file</i>, and may or may not
     * <i>post announcements</i> to the log. If the log must be reset upon
     * restarting from a checkpoint, it will append to the file or erase the
     * file and start over depending on <i>appendOnRestart</i>. If
     * <i>appendOnRestart</i> is false and <i>postAnnouncements</i> is true,
     * then this log will repost all the announcements on restarting from a
     * checkpoint. Returns the position of the log in Output's collection of
     * logs -- you should use this to access the log always; never store the log
     * itself, which may go away upon a system restart. The log can be
     * compressed with gzip, but you cannot appendOnRestart and compress at the
     * same time.
     * 
     * @deprecated Verbosity no longer has an effect.
     */

    public synchronized int addLog(File file, int _verbosity, boolean postAnnouncements, boolean appendOnRestart,
     ...
     * checkpoint. Returns the position of the log in Output's collection of
     * logs -- you should use this to access the log always; never store the log
     * itself, which may go away upon a system restart.
     * 
     * @deprecated Verbosity no longer has an effect
     */
    ...
    /**
     * Creates a new log <!-- of minimal verbosity V_NO_GENERAL-1 --> and adds
     * it to Output. This log will write to the file <i>file</i>, and may or may
     * not <i>post announcements</i> to the log. If the log must be reset upon
     * restarting from a checkpoint, it will append to the file or erase the
     * file and start over depending on <i>appendOnRestart</i>. If
     * <i>appendOnRestart</i> is false and <i>postAnnouncements</i> is true,
     * then this log will repost all the announcements on restarting from a
    ...
    public synchronized int addLog(File file, boolean postAnnouncements, boolean appendOnRestart, boolean gzip)
            throws IOException {
    ...
    /**
     * Creates a new log <!-- of minimal verbosity V_NO_GENERAL-1 --> and adds
     * it to Output. This log will write to the file <i>file</i>, and you may
     * not post announcements to the log. If the log must be reset upon
     * restarting from a checkpoint, it will append to the file or erase the
     * file and start over depending on <i>appendOnRestart</i>. Returns the
     * position of the log in Output's collection of logs -- you should use this
     * to access the log always; never store the log itself, which may go away
     * upon a system restart. The log can be compressed with gzip, but you
     * cannot appendOnRestart and compress at the same time.
     */
    ...
    /**
     * Creates a new log of minimal verbosity <i>verbosity</i> and adds it to
     * Output. This log will write to stdout (descriptor == Log.D_STDOUT) or
     * stderr (descriptor == Log.D_STDERR), and may or may not <i>post
     * announcements</i> to the log. Returns the position of the log in Output's
     * collection of logs -- you should use this to access the log always; never
     * store the log itself, which may go away upon a system restart.
     * 
     * @deprecated Verbosity no longer has an effect
     */
    ...
    /**
     * Creates a new log and adds it to Output. This log will write to stdout
     * (descriptor == Log.D_STDOUT) or stderr (descriptor == Log.D_STDERR), and
     * may or may not <i>post announcements</i> to the log. Returns the position
     * of the log in Output's collection of logs -- you should use this to
     * access the log always; never store the log itself, which may go away upon
     * a system restart.
     */
    ...
    /**
     * Creates a new log of minimal verbosity <i>verbosity</i> and adds it to
     * Output. This log may or may not <i>post announcements</i> to the log, and
     * if it does, it additionally may or may not <i>repost</i> all of its
     * announcements to the log upon a restart. The log writes to <i>writer</i>,
     * which is reset upon system restart by <i>restarter</i>. Returns the
     * position of the log in Output's collection of logs -- you should use this
     * to access the log always; never store the log itself, which may go away
     * upon a system restart.
     * 
    ...
    public synchronized int addLog(Writer writer, LogRestarter restarter, int _verbosity, boolean postAnnouncements,
    ...
    /**
     * Creates a new log <!-- of minimal verbosity V_NO_GENERAL-1 --> and adds
     * it to Output. This log may or may not <i>post announcements</i> to the
     * log, and if it does, it additionally may or may not <i>repost</i> all of
     * its announcements to the log upon a restart. The log writes to
     * <i>writer</i>, which is reset upon system restart by <i>restarter</i>.
     * Returns the position of the log in Output's collection of logs -- you
     * should use this to access the log always; never store the log itself,
     * which may go away upon a system restart.
     */

    public synchronized int addLog(Writer writer, LogRestarter restarter, boolean postAnnouncements,
            boolean repostAnnouncements) {
        logs.addElement(new Log(writer, restarter, postAnnouncements, repostAnnouncements));
        return logs.size() - 1;
    }
    ...
    /**
     * Adds the given log to Output. In general you shouldn't use this method
     * unless you really <i>really</i> need something custom. Returns the
     * position of the log in Output's collection of logs -- you should use this
     * to access the log always; never store the log itself, which may go away
     * upon a system restart.
     */
    ...
    /**
     * Prints an initial error to System.err. This is only to be used by
     * ec.Evolve in starting up the system.
     */
    ...
    /**
     * Prints an initial message to System.err. This is only to be used by
     * ec.Evolve in starting up the system. These messages are not logged.
     */
    ...
    /**
     * Forces one or more file-based logs to reopen, erasing their previous
     * contents. non-file logs ignore this.
     */
    ...
    synchronized void println(String s, int _verbosity, Log log, boolean _announcement, boolean _reposting)
    ...
    /**
     * Prints a message to a given log, with a certain verbosity. If
     * log==ALL_LOGS, posted to all logs which accept announcements.
     * 
     * @deprecated Verbosity no longer has an effect
     */
    synchronized void println(String s, int _verbosity, int log, boolean _announcement) throws OutputException {
        if (log == ALL_LOGS)
            for (int x = 0; x < logs.size(); x++) {
                ...
                if (l == null)
                    throw new OutputException("Unknown log number" + l);
    ...
    /**
     * Prints a non-announcement message to the given logs, with a certain
     * verbosity.
     * 
     * @deprecated Verbosity no longer has an effect
     */
    ...
    /**
     * Prints a non-announcement message to the given logs, with a verbosity of
     * V_NO_GENERAL.
     */
    ...
    /**
     * Prints a non-announcement message to a given log, with a certain
     * verbosity. No '\n' is printed.
     */
    ...
    /**
     * Prints a non-announcement message to a given log, with a certain
     * verbosity. If log==ALL_LOGS, posted to all logs which accept
     * announcements. No '\n' is printed.
     */
    ...
    /**
     * Prints a non-announcement message to a given log<!--, with a verbosity of
     * V_NO_GENERAL-->. If log==ALL_LOGS, posted to all logs which accept
     * announcements. No '\n' is printed.
     */
    ...
    /**
     * Clears out announcements. Note that this will cause these announcements
     * to be unavailable for reposting after a restart!
     */
    ...
    /**
     * Returns a compressing input stream using JZLib
     * (http://www.jcraft.com/jzlib/). If JZLib is not available on your system,
     * this method will return null.
     */
    public static InputStream makeCompressingInputStream(InputStream in) {
        // to do this, we're going to use reflection. But here's the equivalent
        // code:
        /*
         * return new com.jcraft.jzlib.ZInputStream(in);
         */
        try {
            return (InputStream) (Class.forName("com.jcraft.jzlib.ZInputStream").getConstructor(
                    new Class[] { InputStream.class }).newInstance(new Object[] { in }));
        } catch (Exception e) {
            return null;
    ...
    /**
     * Returns a compressing output stream using JZLib
     * (http://www.jcraft.com/jzlib/). If JZLib is not available on your system,
     * this method will return null.
     */
    public static OutputStream makeCompressingOutputStream(OutputStream out) {
        ...
        // code:
        /*
         * com.jcraft.jzlib.ZOutputStream stream = new
         * com.jcraft.jzlib.ZOutputStream(out,
         * com.jcraft.jzlib.JZlib.Z_BEST_SPEED);
         * stream.setFlushMode(com.jcraft.jzlib.JZlib.Z_SYNC_FLUSH); return
         * stream;
         */
        try {
            ...
            Object outi = outc.getConstructor(new Class[] { OutputStream.class, Integer.TYPE }).newInstance(
                    new Object[] { out, new Integer(Z_BEST_SPEED) });
            outc.getMethod("setFlushMode", new Class[] { Integer.TYPE }).invoke(outi,
                    new Object[] { new Integer(Z_SYNC_FLUSH) });
    ...
    class Announcement implements Serializable {
        ...
        /**
         * Creates a new announcement with text <i>t</i> and verbosity value
         * <i>v</i>
         */
