 ...
 * 
 * Logs can be <i>restarted</i> after a computer outage by using the information
 * stored in the LogRestarter <tt>restarter</tt>.
 * 
 * @author Sean Luke
 * @version 1.0
 */
...
public class Log implements Serializable {
    ...
    /**
     * If the log writes to a file, should it append to the file on restart, or
     * should it overwrite the file?
     */
    ...
    /**
     * Creates a log to a given filename; this file may or may not be appended
     * to on restart, depending on _appendOnRestart. If and only if the file is
     * <i>not</i> appended to on restart, then announcements are reposted on
     * restart.
     */
     ...
     * to on restart, depending on _appendOnRestart. If and only if the file is
     * <i>not</i> appended to on restart, then announcements are reposted on
     * restart. The file can be compressed with gzip, but you may not both gzip
     * AND appendOnRestart. If gzipped, then .gz is automagically appended to
     * the file name.
     */
        ...
        if (gzip) {
            ...
            if (appendOnRestart)
                throw new IOException(
                        "Cannot gzip and appendOnRestart at the same time: new Log(File,int,boolean,boolean,boolean)");

            writer = new PrintWriter(new OutputStreamWriter(new GZIPOutputStream(new BufferedOutputStream(
            ...
            restarter = new LogRestarter() {
                ...
                public Log reopen(Log l) throws IOException {
                    if (l.writer != null && !l.isLoggingToSystemOut)
                        l.writer.close();
                    l.writer = new PrintWriter(new OutputStreamWriter(new GZIPOutputStream(new BufferedOutputStream(
                            new FileOutputStream(l.filename)))));
                    return l;
                }
            };
        }
            ...
            restarter = new LogRestarter() {
                public Log restart(Log l) throws IOException {
                    l.writer = new PrintWriter(new BufferedWriter(new FileWriter(l.filename.getPath(),
                            l.appendOnRestart)));
                    return l;
                }

                public Log reopen(Log l) throws IOException {
                    if (l.writer != null && !isLoggingToSystemOut)
                        l.writer.close();
    ...
    /**
     * Creates a log on stdout (descriptor == Log.D_STDOUT) or stderr
     * (descriptor == Log.D_STDERR).
     */
    ...
    /**
     * Creates a log on a given Writer and custom LogRestarter. In general, You
     * should not use this to write to a file. Use Log(_filename... instead.
     */

    public Log(Writer _writer, LogRestarter _restarter, boolean _postAnnouncements,
    ...
    /**
     * Restarts a log after a system restart from checkpoint. Returns the
     * restarted log -- note that it may not be the same log!
     */
