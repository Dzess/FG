 ...
 * 
 * <p>
 * A Parameter is an object which the ParameterDatabase class uses as a key to
 * associate with strings, forming a key-value pair. Parameters are designed to
 * be hierarchical in nature, consisting of "path items" separated by a path
 * separator. Parameters are created either from a single path item, from an
 * array of path items, or both. For example, a parameter with the path
...
public class Parameter implements Serializable {
    ...
    /**
     * Creates a new parameter by joining the path items in s into a single
     * path.
     */
    ...
    /**
     * Creates a new parameter from the path item in s, plus the path items in
     * s2. s2 may be null or empty, but not s
     */
    ...
    /**
     * Returns a new parameter with s added to the end of the current path
     * items.
     */
     ...
     * would result in a parameter with an empty collection of path items, null
     * is returned.
     */
    ...
    /**
     * Returns a new parameter with n path items popped off the end. If this
     * would result in a parameter with an empty collection of path items, null
     * is returned.
     */
