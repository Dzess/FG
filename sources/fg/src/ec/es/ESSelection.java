...
public class ESSelection extends SelectionMethod {
	...
	public int produce(final int subpopulation, final EvolutionState state, final int thread) {
		...
		int pos = (breeder.lambda[subpopulation] % state.breedthreads == 0 ? breeder.lambda[subpopulation] / state.breedthreads
				: breeder.lambda[subpopulation] / state.breedthreads + 1) * thread + breeder.count[thread]; // note
																											// integer
																											// division
