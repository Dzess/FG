...
/**
 * GenerationSwitchPipeline is a simple BreedingPipeline which switches its
 * source depending on the generation. If the generation number is < n, then
 * GenerationSwitchPipeline uses source.0. If the generation number if >= n,
 * then GenerationSwitchPipeline uses source.1.
 * 
 * <p>
 * <b>Important Note:</b> Because GenerationSwitchPipeline gets the generation
 * number from the EvolutionState, and this number is not protected by a mutex,
 * if you create an EvolutionState or Breeder which uses multiple threads that
 * can update the generation number as they like, you could cause a race
 * condition. This doesn't occur with the present EvolutionState objects
 * provided with ECJ, but you should be aware of the possibility.
 * 
 * <p>
 ...
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>generate-max</tt><br>
 * <font size=-1> bool = <tt>true</tt> (default) or <tt>false</tt></font></td>
 * <td valign=top>(Each time produce(...) is called, should the
 * GenerationSwitchPipeline force all its sources to produce exactly the same
 * number of individuals as the largest typical number of individuals produced
 * by any source in the group?)</td>
 * </tr>
 * <tr>
 * <td valign=top><i>base</i>.<tt>switch-at</tt><br>
 * <font size=-1> int &gt;= 0</tt></font></td>
 * <td valign=top>(The generation we will switch at)</td>
 * </tr>
 * </table>
 * 
 * <p>
