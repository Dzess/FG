...
/**
 * If empty, a BufferedBreedingPipeline makes a request of exactly
 * <i>num-inds</i> individuals from a single child source; it then uses these
 * individuals to fill requests (returning min each time), until the buffer is
 * emptied, at which time it grabs exactly <i>num-inds</i> more individuals, and
 * so on.
 * 
 * <p>
 * What is this useful for? Well, let's say for example that you want to cross
 * over two individuals, then cross them over again. You'd like to hook up two
 * CrossoverPipelines in series. Unfortunately, CrossoverPipeline takes two
 * sources; even if you set them to the same source, it requests <i>one</i>
 * individual from the first source and then <i>one</i> from the second, where
 * what you really want is for it to request <i>two</i> individuals from a
 * single source (the other CrossoverPipeline).
 * 
 * <p>
 * The solution to this is to hook a CrossoverPipeline as the source to a
 * BufferedBreedingPipeline of buffer-size 2 (or some multiple of 2 actually).
 * Then the BufferedBreedingPipeline is set as both sources to another
 * CrossoverPipeline.
 * 
 * <p>
 ...
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>num-inds</tt><br>
 * <font size=-1>int &gt;= 1</font></td>
 * <td valign=top>(the buffer size)</td>
 * </tr>
 * </table>
 * 
 * <p>
