 ...
 * 
 * <p>
 * The purpose of a prototype is to make it possible to ask classes, determined
 * at run-time by user parameters, to instantiate themselves very many times
 * without using the Reflection library, which would be very inefficient.
 * 
 * <p>
 * ECJ makes extensive use of Prototypes. Individuals are prototypes. Species
 * are prototypes. Fitness objects are prototypes. Breeding pipelines and
 * selection methods are prototypes. In the GP section, GPNodes and GPTrees are
 * prototypes. In the Rule section, Rulesets and Rules are prototypes. In the
 * Vector section, VectorGenes are prototypes. And so on.
 * 
 * <p>
 * ECJ uses Prototypes almost exclusively instead of calling <tt>new</tt>. This
 * is because <tt>new</t> requires that you know, in your code, the exact
 ...
 * 
 * <p>Sadly, clone() is rather slower than calling <tt>new</tt>. However it
 * <i>is</i> a lot faster than calling java.lang.Class.newInstance(), and
 * somewhat faster than rolling our own "cloner" method.
 * 
 * <p>
 * Prototypes must be Cloneable, Serializable (through Setup), and of course,
 * Setup.
 * 
 * @author Sean Luke
 * @version 1.0
