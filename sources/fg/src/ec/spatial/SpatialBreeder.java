 ...
 * using a generational approach. A SpatialBreeder may have multiple threads; it
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		for (int i = 0; i < elite.length; i++)
			if (elite[i] > 0) {
				state.output
						.warning(
								"You're using elitism with SpatialBreeder.  This is unwise as elitism is done by moving individuals around in the population, thus messing up the spatial nature of breeding.",
		...
		for (int subpop = 0; subpop < newpop.subpops.length; subpop++) {
			...
			if (!bp.produces(state, newpop, subpop, threadnum))
				state.output.fatal("The Breeding Pipeline of subpopulation " + subpop
						+ " does not produce individuals of the expected species " + newpop.subpops[subpop].species.getClass().getName()
