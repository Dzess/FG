 ...
 * 
 * <p>
 * MultiPopCoevolutionaryEvaluator is an Evaluator which performs <i>competitive
 * or cooperative multi-population coevolution</i>. Competitive coevolution is
 * where individuals' fitness is determined by testing them against individuals
 * from other subpopulation. Cooperative coevolution is where individuals form
 * teams together with members of other subpopulations, and the individuals'
 * fitness is computed based on the performance of such teams. This evaluator
 * assumes that the problem can only evaluate groups of individuals containing
 * one individual from each subpopulation. Individuals are evaluated regardless
 * of whether or not they've been evaluated in the past.
 * 
 * <p>
 * Your Problem is responsible for updating up the fitness appropriately with
 * values usually obtained from teaming up the individual with different
 * partners from the other subpopulations. MultiPopCoevolutionaryEvaluator
 * expects to use Problems which adhere to the GroupedProblemForm interface,
 * which defines a new evaluate(...) function, plus a preprocess(...) and
 * postprocess(...) function.
 * 
 * <p>
 * This coevolutionary evaluator is single-threaded -- maybe we'll hack in
 * multithreading later. It allows any number of subpopulations (implicitly, any
 * number of individuals being evaluated together). The order of individuals in
 * the subpopulation may be changed during the evaluation process.
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 * <tr>
 * <td valign=top><i>base.</i><tt>subpop.num-current</tt><br>
 * <font size=-1> int &gt;= 0</font></td>
 * <td valign=top>(the number of random individuals from any given subpopulation
 * to be selected as collaborators)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>subpop.num-elites</tt><br>
 * <font size=-1> int &gt;= 0</font></td>
 * <td valign=top>(the number of elite individuals from any given subpopulation
 * to be selected as collaborators. For generation 0, random individuals from
 * the current population will be used. )</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>subpop.num-prev</tt><br>
 * <font size=-1> int &gt;= 0</font></td>
 * <td valign=top>(the number of random individuals from any given subpopulation
 * from the previous population to be selected as collaborators. For generation
 * 0, random individuals from the current population will be used)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>subpop.X.select-prev</tt><br>
 * <font size=-1> instance of ec.SelectionMethod</font></td>
 * <td valign=top>(the SelectionMethod used to select partners from the
 * individuals in subpopulation X at the previous generation)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>subpop.X.select-current</tt><br>
 * <font size=-1> instance of ec.SelectionMethod</font></td>
 * <td valign=top>(the SelectionMethod used to select partners from the
 * individuals in subpopulation X at the current generation. <b>WARNING.</b>
 * This SelectionMethod must not select based on fitness, since fitness hasn't
 * been set yet. RandomSelection is a good choice. )</td>
 * </tr>
 * 
 * 
 * </table>
...
public class MultiPopCoevolutionaryEvaluator extends Evaluator {
    ...
    // the number of random partners selected from the current and previous
    // generations
    ...
    // the selection method used to select the other partners from the current
    // generation
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (numCurrent < 0)
            ...
            for (int i = 0; i < numSubpopulations; i++) {
                selectionMethodCurrent[i] = (SelectionMethod) (state.parameters.getInstanceForParameter(
                        base.push(P_SUBPOP).push("" + i).push(P_SELECTION_METHOD_CURRENT),
                        base.push(P_SELECTION_METHOD_CURRENT), SelectionMethod.class));
                if (selectionMethodCurrent[i] == null)
                    ...
                    selectionMethodCurrent[i].setup(state,
                            base.push(P_SUBPOP).push("" + i).push(P_SELECTION_METHOD_CURRENT));
            }
        }
        ...
        if (numPrev < 0)
            ...
            for (int i = 0; i < numSubpopulations; i++) {
                selectionMethodPrev[i] = (SelectionMethod) (state.parameters.getInstanceForParameter(base
                        .push(P_SUBPOP).push("" + i).push(P_SELECTION_METHOD_PREV), base.push(P_SELECTION_METHOD_PREV),
    ...
    protected void beforeCoevolutionaryEvaluation(final EvolutionState state, final Population population,
            final GroupedProblemForm prob) {
        if (state.generation == 0) {
            //
            // create arrays for the elite individuals in the population at the
            // previous generation.
            // deep clone the elite individuals as random individuals (in the
            // initial generation, nobody has been evaluated yet).
            //

            // deal with the elites
            eliteIndividuals = new Individual[state.population.subpops.length][numElite];
            // copy the first individuals in each subpopulation (they are
    ...
    public void performCoevolutionaryEvaluation(final EvolutionState state, final Population population,
            final GroupedProblemForm prob) {
            ...
            for (int i = 0; i < state.population.subpops[j].individuals.length; i++) {
                ...
                for (int k = 0; k < eliteIndividuals[j].length; k++) {
                    for (int ind = 0; ind < inds.length; ind++) {
                        if (ind == j) {
                            inds[ind] = state.population.subpops[j].individuals[i];
                            updates[ind] = true;
                        } else {
                ...
                for (int k = 0; k < numCurrent; k++) {
                    for (int ind = 0; ind < inds.length; ind++) {
                        if (ind == j) {
                            ...
                            inds[ind] = produce(selectionMethodCurrent[j], j, i, state, 0);
                            updates[ind] = false;
                        }
                    }
                    prob.evaluate(state, inds, updates, false, subpops, 0);
                ...
                for (int k = 0; k < numCurrent; k++) {
                    for (int ind = 0; ind < inds.length; ind++) {
                        if (ind == j) {
                            ...
                            inds[ind] = state.population.subpops[j].individuals[state.random[0]
                                    .nextInt(state.population.subpops[j].individuals.length)];
                            updates[ind] = false;
                        }
                    }
                    prob.evaluate(state, inds, updates, false, subpops, 0);
    ...
    protected Individual produce(SelectionMethod method, int subpopulation, int individual, EvolutionState state,
            int thread) {
        return state.population.subpops[subpopulation].individuals[method.produce(subpopulation, state, thread)];
    }

    protected void afterCoevolutionaryEvaluation(final EvolutionState state, final Population population,
            final GroupedProblemForm prob) {
        if (numElite > 0) {
    ...
    void loadElites(final EvolutionState state, final Subpopulation subpop, int whichSubpop) {
        if (numElite == 1) {
            int best = 0;
            ...
            for (int x = 1; x < oldinds.length; x++)
                if (oldinds[x].fitness.betterThan(oldinds[best].fitness))
                    best = x;
            eliteIndividuals[whichSubpop][0] = (Individual) (state.population.subpops[whichSubpop].individuals[best]
                    .clone());
        } else if (numElite > 0) // we'll need to sort
        {
            ...
            for (int j = 0; j < numElite; j++)
                eliteIndividuals[whichSubpop][j] = (Individual) (state.population.subpops[whichSubpop].individuals[orderedPop[j]]
                        .clone());
        }
    }
