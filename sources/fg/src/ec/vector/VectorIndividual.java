...
/**
 * VectorIndividual is the abstract superclass of simple individual
 * representations which consist of vectors of values (booleans, integers,
 * floating-point, etc.)
 * 
 * <p>
 * This class contains two methods, defaultCrossover and defaultMutate, which
 * can be overridden if all you need is a simple crossover and a simple mutate
 * mechanism. the VectorCrossoverPipeline and VectorMutationPipeline classes use
 * these methods to do their handiwork. For more sophisticated crossover and
 * mutation, you'll need to write a custom breeding pipeline.
 * 
 * <p>
 * The <i>kind</i> of default crossover and mutation, and associated
 * information, is stored in the VectorIndividual's VectorSpecies object, which
 * is obtained through the <tt>species</tt> variable. For example,
 * VectorIndividual assumes three common types of crossover as defined in
 * VectorSpecies which you should implement in your defaultCrossover method:
 * one-point, two-point, and any-point (otherwise known as "uniform") crossover.
 * 
 * <p>
 * VectorIndividual is typically used for fixed-length vector representations;
 * however, it can also be used with variable-length representations. Two
 * methods have been provided in all subclasses of VectorIndividual to help you
 * there: split and join, which you can use to break up and reconnect
 * VectorIndividuals in a variety of ways. Note that you may want to override
 * the reset() method to create individuals with different initial lengths.
 * 
 * <p>
 * VectorIndividuals must belong to the species VectorSpecies (or some subclass
 * of it).
 * 
 * 
 * <P>
...
public abstract class VectorIndividual extends Individual {
     ...
     * manner. In most implementations provided in ECJ, one-, two-, and
     * any-point crossover is done with a for loop, rather than a possibly more
     * efficient approach like arrayCopy(). The disadvantage is that arrayCopy()
     * takes advantage of a CPU's bulk copying. The advantage is that
     * arrayCopy() would require a scratch array, so you'd be allocing and GCing
     * an array for every crossover. Dunno which is more efficient.
     */
    ...
    /**
     * Returns the gene array. If you know the type of the array, you can cast
     * it and work on it directly. Otherwise, you can still manipulate it in
     * general, because arrays (like all objects) respond to clone() and can be
     * manipulated with arrayCopy without bothering with their type. This might
     * be useful in creating special generalized crossover operators -- we
     * apologize in advance for the fact that Java doesn't have a template
     * system. :-( The default version returns null.
     */
    ...
    /**
     * Initializes the individual to a new size. Only use this if you need to
     * initialize variable-length individuals.
     */
    ...
    /**
     * Sets the genome length. If the length is longer, then it is filled with a
     * default value (likely 0 or false). This may or may not be a valid value
     * -- you will need to set appropriate values here. The default
     * implementation does nothing; but all subclasses in ECJ implement a subset
     * of this.
     */
     ...
     * sorted. pieces.length must be 1 + points.length. The default form does
     * nothing -- be careful not to use this method if it's not implemented! It
     * should be trivial to implement it for your genome -- just like at the
    ...
    /**
     * Joins the n pieces and sets the genome to their concatenation. The
     * default form does nothing. It should be trivial to implement it for your
     * genome -- just like at the other implementations.
     */
     ...
     * Does NOT copy the array, but modifies it in place. If the
     * VectorIndividual holds numbers or booleans etc. instead of genes, nothing
     * is cloned (why bother?).
     */
