 ...
 * vector is between the values of that component of the parent vectors.</li>
 * </ul>
 * <p>
 * Note that for LongVectorIndividuals, there are certain values that will never
 * be created by line and intermediate recombination, because the recombination
 * is calculated using doubles and then rounded to the nearest long. For large
 * enough values (but still smaller than the maximum long), the difference
 * between one double and the next is greater than one.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * If the algorithm used is the geometric distribution, the VectorSpecies starts
 * at a minimum size and continues flipping a coin with a certain
 * "resize probability", increasing the size each time, until the coin comes up
 * tails (fails). The chunk size must be 1 in this case.
 * 
 * <p>
 * If the algorithm used is the uniform distribution, the VectorSpecies picks a
 * random size between a provided minimum and maximum size, inclusive. The chunk
 * size must be 1 in this case.
 * 
 * <p>
 * If the size is fixed, then you can also provide a "chunk size" which
 * constrains the locations in which crossover can be performed (only along
 * chunk boundaries). The genome size must be a multiple of the chunk size in
 * this case.
 * 
 * <p>
 * VectorSpecies also contains a number of parameters guiding how the individual
 ...
 * <td valign=top>(size of the genome, or if 'geometric' or 'uniform', the
 * algorithm used to size the initial genome)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>chunk-size</tt><br>
 * <font size=-1>1 &lt;= int &lt;= genome-size (default=1)</font></td>
 * <td valign=top>(the chunk size for crossover (crossover will only occur on
 * chunk boundaries))</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>geometric-prob</tt><br>
 * <font size=-1>0.0 &lt;= float &lt; 1.0</font></td>
 * <td valign=top>(the coin-flip probability for increasing the initial size
 * using the geometric distribution)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>min-initial-size</tt><br>
 * <font size=-1>int &gt;= 0</font></td>
 * <td valign=top>(the minimum initial size of the genome)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>max-initial-size</tt><br>
 * <font size=-1>int &gt;= min-initial-size</font></td>
 * <td valign=top>(the maximum initial size of the genome)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>crossover-type</tt><br>
 * <font size=-1>string, one of: one, two, any</font></td>
 * <td valign=top>(default crossover type (one-point, two-point, any-point
 * (uniform), line, or intermediate)</td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>crossover-prob</tt><br>
 * <font size=-1>0.0 &gt;= float &gt;= 1.0 </font></td>
 * <td valign=top>(probability that a gene will get crossed over during
 * any-point crossover)</td>
 * </tr>
 * 
 * <tr>
 ...
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>line-extension</tt><br>
 * <font size=-1>float &gt;= 0.0 </font></td>
 * <td valign=top>(for line and intermediate recombination, how far along the
 * line or outside of the hypercube children can be. If this value is zero, all
 * children must be within the hypercube.)
 * 
 * </table>
 * 
 * <p>
