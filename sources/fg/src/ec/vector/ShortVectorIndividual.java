...
/**
 * ShortVectorIndividual is a VectorIndividual whose genome is an array of
 * shorts. Gene values may range from species.mingene(x) to species.maxgene(x),
        ...
        if (s.mutationProbability > 0.0)
            for (int x = 0; x < genome.length; x++)
                if (state.random[thread].nextBoolean(s.mutationProbability))
                    genome[x] = (short) ((int) s.minGene(x) + state.random[thread].nextInt((int) s.maxGene(x)
                            - (int) s.minGene(x) + 1));
    }

    /**
     * Initializes the individual by randomly choosing Shorts uniformly from
     * mingene to maxgene.
     */
