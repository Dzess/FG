...
/**
 * Picks individuals in a population using the Stochastic Universal Selection
 * (SUS) process, using fitnesses as returned by their fitness() methods. This
 * is expensive to set up and bring down, so it's not appropriate for
 * steady-state evolution. If you're not familiar with the relative advantages
 * of selection methods and just want a good one, use TournamentSelection
 * instead. Not appropriate for multiobjective fitnesses.
 * 
 * <p>
 * By default this implementation of SUS shuffles the order of the individuals
 * in the distribution before performing selection. This isn't always present in
 * classic implementations of the algorithm but it can't hurt anything and
 * certainly can avoid certain pathological situations. If you'd prefer not to
 * preshuffle, set shuffle=false Note that we don't actually change the order of
 * the individuals in the population -- instead we maintain our own internal
 * array of indices and shuffle that.
 * 
 * <p>
 * Like truncation selection, SUS samples N individuals (with replacement) up
 * front from the population, Then returns those individuals one by one. ECJ's
 * implementation assumes that N is the size of the population -- that is,
 * you're going to ultimately request a whole population out of this one
 * selection method. This could be a false assumption: for example, if you only
 * sometimes call this selection method, and sometimes TournamentSelection; or
 * if you've got multiple pipelines. In these cases, SUS is probably a bad
 * choice anyway.
 * 
 * <p>
 * If you ask for <i>more</i> than a population's worth of individuals, SUS
 * tries to handle this gracefully by reshuffling its array and starting to
 * select over again. But again that might suggest you are doing something
 * wrong.
 * 
 * <p>
 ...
 * <table>
 * <tr>
 * <td valign=top><i>base.</i><tt>shuffle</tt><br>
 * <font size=-1> bool = <tt>true</tt> (default) or <tt>false</tt></font></td>
 * <td valign=top>(should we preshuffle the array before doing selection?)</td>
 * </tr>
 * 
 * </table>
 * <p>
