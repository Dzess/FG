...
/**
 * Picks individuals in a population in direct proportion to their fitnesses as
 * returned by their fitness() methods. This is expensive to set up and bring
 * down, so it's not appropriate for steady-state evolution. If you're not
 * familiar with the relative advantages of selection methods and just want a
 * good one, use TournamentSelection instead. Not appropriate for multiobjective
 * fitnesses.
 * 
 * <p>
    ...
    public void prepareToProduce(final EvolutionState s, final int subpopulation, final int thread) {
        ...
        for (int x = 0; x < fitnesses.length; x++) {
            ...
            if (fitnesses[x] < 0) // uh oh
                s.output.fatal("Discovered a negative fitness value.  FitProportionateSelection requires that all fitness values be non-negative(offending subpopulation #"
                        + subpopulation + ")");
        }
