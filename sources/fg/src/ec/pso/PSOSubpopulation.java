 ...
 * 
 * <p>
 * Particle Swarm Optimization (PSO) is a population-oriented stochastic search
 * technique similar to genetic algorithms, evolutionary strategies, and other
 * evolutionary computation algorithms. The technique discovers solutions for
 * N-dimensional parameterized problems: basically it discovers the point in
 * N-dimensional space which maximizes some quality function.
 * 
 * <p>
 ...
 * <td valign=top>(the number of individuals per neighborhood)<br>
 * </td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>clamp</tt><br>
 * <font size=-1>boolean</font></td>
 * <td valign=top>(clamp the individual to stay within the bounds or not)<br>
 * </td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>initial-velocity-scale</tt><br>
 * <font size=-1>double</font></td>
 * <td valign=top>(particles are initialized with a random velocity and this
 * value provides bounds. A value of 1.0 means that the velocity will be within
 * +/- the range of the genotype.)<br>
 * </td>
 * </tr>
 * 
 * <tr>
 * <td valign=top><i>base</i>.<tt>velocity-multiplier</tt><br>
 * <font size=-1>double</font></td>
 * <td valign=top>(particle velocities are multiplied by this value before the
 * particle is updated. Increasing this value helps particles to escape local
 * optima, but slows convergence. The default value of 1.5 is geared toward
 * multi-modal landscapes.)<br>
 * </td>
 * </tr>
 * 
 * </table>
...
public class PSOSubpopulation extends Subpopulation {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (!(species instanceof FloatVectorSpecies))
            state.output.error(
                    "PSOSubpopulation requires that its species is ec.vector.FloatVectorSpecies or a subclass.  Yours is: "
                            + species.getClass(), null, null);
        if (!(species.i_prototype instanceof DoubleVectorIndividual))
            state.output
                    .error("PSOSubpopulation requires that its species' prototypical individual be is ec.vector.DoubleVectorSpecies or a subclass.  Yours is: "
                            + species.getClass(), null, null);
    ...
    public void populate(EvolutionState state, int thread) {
        ...
        if (loadInds == null) // we're generating new individuals, not reading
                              // them from a file
        {
            ...
            for (int i = 0; i < individuals.length; i++) {
                ...
                for (int j = 0; j < prevInd.genomeLength(); j++) {
                    ...
                    do
                        prevInd.genome[j] = val + (range * initialVelocityScale)
                                * (state.random[thread].nextDouble() * 2.0 - 1.0);
    ...
    /**
     * Overridden to include the global best, neighborhood bests, personal
     * bests, and previous individuals in the stream. The neighborhood size,
     * clamp range, and initial velocity scale are not included -- it's assumed
     * you're using the same values for them on reading, or understand that the
     * values are revised.
     */
    ...
    DoubleVectorIndividual possiblyReadIndividual(final EvolutionState state, final LineNumberReader reader)
            throws IOException {
    ...
    public void readSubpopulation(final EvolutionState state, final DataInput dataInput) throws IOException {
        ...
        // neighborhoodBests
        for (int i = 0; i < individuals.length; i++)
            neighborhoodBests[i] = (dataInput.readBoolean() ? (DoubleVectorIndividual) species.newIndividual(state,
                    dataInput) : null);

        // personalBests
        for (int i = 0; i < individuals.length; i++)
            personalBests[i] = (dataInput.readBoolean() ? (DoubleVectorIndividual) species.newIndividual(state,
                    dataInput) : null);

        // previous Individuals
        for (int i = 0; i < individuals.length; i++)
            previousIndividuals[i] = (dataInput.readBoolean() ? (DoubleVectorIndividual) species.newIndividual(state,
                    dataInput) : null);
