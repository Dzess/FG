 ...
 * control, whereby some <i>kill-proportion</i> of above-average-sized
 * individuals in each subpopulation have their fitnesses set to a very bad
 * value, and marks them as already evaluated (so the Evaluator can skip them).
 * The specific individuals in this proportion is determined at random.
 * 
 * <p>
 * Different Fitnesses have different meanings of the word "bad". At present, we
 * set the fitness to -Float.MAX_VALUE if it's a SimpleFitness, and set it to
 * Float.MAX_VALUE if it's a KozaFitnesss. If it's any other kind of Fitness, an
 * error is reported. You can override the "bad-setter" function
 * setMinimumFitness(...) to make other kinds of fitness bad in different ways.
 * In the future we may revisit how to set Fitnesses to "bad" in a more general
 * way if this becomes an issue.
 * 
 * <p>
 * Tarpeian is implemented as a Statistics. Why? Because we need to mark
 * individuals as evaluated prior to the Evaluator getting to them, and also
 * need to keep track of the total proportion marked as such. We considered
 * doing this as a SelectionMethod, as a BreedingPipeline, as a Breeder, and as
 * an Evaluator. None are good options really -- Evaluator is the best approach
 * but it means we have special Tarpeian Evaluators, so it's no longer
 * orthogonal with other Evaluators. Eventually we settled on the one object
 * which has the right hooks and can be easily stuck onto the system without
 * modifying anything in a special-purpose way: a Statistics object.
 * 
 * <p>
 * All you need to do is add TarpeianStatistics as a child to your existing
 * Statistics chain. If you have one existing Statistics, then you just add the
 * parameters <tt>stat.num-children=1</tt> and
 * <tt>stat.child.0=ec.parsimony.TarpeianStatistics</tt> You'll also need to
 * specify the kill proportion (for example,
 * <tt>stat.child.0.kill-proportion=0.2</tt> )
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>kill-proportion</tt><br>
 * <font size=-1>0 &lt; int &lt; 1</font></td>
 * <td valign=top>(proportion of above-average-sized individuals killed)</td>
 * </tr>
 * </table>
 */
