 ...
 * 
 * <p>
 * Ratio Bucket Lexicographic Tournament selection works like as follows. The
 * sizes of buckets are proportioned so that low-fitness individuals are placed
 * into much larger buckets than high-fitness individuals. A bucket ratio
 * <i>1/ratio</i> is specified beforehand. The bottom <i>1/ratio</i> individuals
 * of the population are placed into the bottom bucket. If any individuals
 * remain in the population with the same fitness as the best individual in the
 * bottom bucket, they too are placed in that bucket. Of the remaining
 * population, the next <i>1/ratio</i> individuals are placed into the next
 * bucket, plus any individuals remaining in the population with the same
 * fitness as the best individual now in that bucket, and so on. This continues
 * until every member of the population has been placed in a bucket. Once again,
 * the fitness of every individual in a bucket is set to the rank of the bucket
 * relative to other buckets. Ratio bucketing thus allows parsimony to have more
 * of an effect on average when two similar low-fitness individuals are
 * considered than when two high-fitness individuals are considered.
 * 
 ...
 * <td valign=top>(the ratio of worst out of remaining individuals that go in
 * the next bucket)</td>
 * </tr>
 * </table>
 * 
...
public class RatioBucketTournamentSelection extends SelectionMethod implements SteadyStateBSourceForm {
    ...
    /**
     * The value of RATIO: each step, the worse 1/RATIO individuals are assigned
     * the same fitness
     */
        ...
        for (int i = 1; i < state.population.subpops[subpopulation].individuals.length; i++) {
            ...
            } else // check if it has the same fitness as last individual
            {
                if (((Individual) state.population.subpops[subpopulation].individuals[i]).fitness
                    ...
                    bucketValues[i] = bucketValues[i - 1] - 1; // decrease the
                                                               // fitness, so
                                                               // that high fit
                                                               // individuals
                                                               // have lower
