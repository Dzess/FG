...
/**
 * 
 * Does a tournament selection, limited to the subpopulation it's working in at
 * the time.
 * 
 * <p>
 * Bucket Lexicographic Tournament selection works like as follows. There is a
 * number of buckets (<i>num-buckets</i>) specified beforehand, and each is
 * assigned a rank from 1 to <i>num-buckets</i>. The population, of size
 * <i>pop-size</i>, is sorted by fitness. The bottom
 * <i>pop-size</i>/<i>num-buckets</i> individuals are placed in the worst ranked
 * bucket, plus any individuals remaining in the population with the same
 * fitness as the best individual in the bucket. Then the second worst
 * <i>pop-size</i>/<i>num-buckets</i> individuals are placed in the second worst
 * ranked bucket, plus any individuals in the population equal in fitness to the
 * best individual in that bucket. This continues until there are no individuals
 * in the population. Note that the topmost bucket with individuals can hold
 * fewer than <i>pop-size</i>/<i>num-buckets</i> individuals, if <i>pop-size</i>
 * is not a multiple of <i>num-buckets</i>. Depending on the number of
 * equal-fitness individuals in the population, there can be some top buckets
 * that are never filled. The fitness of each individual in a bucket is set to
 * the rank of the bucket holding it. Direct bucketing has the effect of trading
 * off fitness differences for size. Thus the larger the bucket, the stronger
 * the emphasis on size as a secondary objective.
 * 
 * After ranking the individuals, <i>size</i> individuals are chosen at random
 * from the population. Of those individuals, the one with the highest rank is
 * selected. If the two individuals are in the same rank, meaning that they have
 * similar fitness, the one with the smallest size is selected.
 * 
 * <p>
 * Bucket Lexicographic Tournament selection is so simple that it doesn't need
 * to maintain a cache of any form, so many of the SelectionMethod methods just
 ...
 * 
 * <tr>
 * <td valign=top><i>base.</i><tt>num-buckets</tt><br>
 * <font size=-1>int &gt;= 1 (default 10)</font></td>
 * <td valign=top>(the number of buckets)</td>
 * </tr>
 * </table>
 * 
 * <p>
...
public class BucketTournamentSelection extends SelectionMethod implements SteadyStateBSourceForm {
    ...
    // the indexes of the buckets where the individuals should go (will be used
    // instead of fitness)
    int[] bucketValues;
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        if (state.parameters.exists(base.push(P_BUCKETS), def.push(P_BUCKETS))) {
            ...
            if (nBuckets < 1) {
                state.output.fatal("The number of buckets size must be >= 1.", base.push(P_BUCKETS),
                        def.push(P_BUCKETS));
            }
        } else {
    ...
    public void prepareToProduce(final EvolutionState state, final int subpopulation, final int thread) {
        ...
        // correct?
        java.util.Arrays.sort(state.population.subpops[subpopulation].individuals, new java.util.Comparator() {
        ...
        float averageBuck = ((float) state.population.subpops[subpopulation].individuals.length) / ((float) nBuckets);
        ...
        for (int i = 1; i < state.population.subpops[subpopulation].individuals.length; i++) {
            // if there is still some place left in the current bucket, throw
            // the current individual there too
            if (nInd < averageBuck) {
                bucketValues[i] = bucketValues[i - 1];
                nInd++;
            } else // check if it has the same fitness as last individual
            {
                if (((Individual) state.population.subpops[subpopulation].individuals[i]).fitness
                        .equivalentTo(((Individual) state.population.subpops[subpopulation].individuals[i - 1]).fitness)) {
                    // now the individual has exactly the same fitness as
                    // previous one,
                    // so we just put it in the same bucket as the previous
                    // one(s)
                    bucketValues[i] = bucketValues[i - 1];
                    nInd++;
                } else {
                    ...
                    if (bucketValues[i - 1] + 1 < nBuckets) {
                        ...
                        nInd = 1;
                    } else // no more buckets left, just stick everything in the
                           // last bucket
                    {
                        bucketValues[i] = bucketValues[i - 1];
                        nInd++;
