...
public class DoubleTournamentSelection extends SelectionMethod implements SteadyStateBSourceForm {
    ...
    /**
     * What's our probability of selection? If 1.0, we always pick the "good"
     * individual.
     */
    ...
    /**
     * Produces the index of a person selected from among several by a
     * tournament. The tournament's criteria is fitness of individuals if
     * doLengthFirst is true, otherwise the size of the individuals.
     */
    public int produce(final int subpopulation, final EvolutionState state, final int thread) {
        int[] inds = new int[size2];
        for (int x = 0; x < size2; x++)
            inds[x] = make(subpopulation, state, thread);
    ...
    /**
     * Produces the index of a person selected from among several by a
     * tournament. The tournament's criteria is size of individuals if
     * doLengthFirst is true, otherwise the fitness of the individuals.
     */
    public int make(final int subpopulation, final EvolutionState state, final int thread) {
        if (doLengthFirst) // if length first, the first tournament is based on
                           // size
        {
            // pick size random individuals, then pick the best.
            Individual[] oldinds = state.population.subpops[subpopulation].individuals;
