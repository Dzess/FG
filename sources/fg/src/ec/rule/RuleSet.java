...
/**
 * RuleSet is a set of Rules, implemented straightforwardly as an
 * arbitrary-length array of Rules. A RuleIndividual is simply a list of
 * RuleSets. Most typically, a RuleIndividual contains a single RuleSet,
 * containing a variety of Rules. RuleSets contain many useful subsetting and
 * modification functions which you can use in breeding operators which modify
 * RuleSets and Rules.
 * 
 * <p>
 * Besides the Rules themselves, the only thing else a RuleSet contains is a
 * pointer to a corresponding RuleSetConstraints object, which holds all of its
 * modification parameters. See RuleSetConstraints for a description of these
 * parameters.
 * 
 * <p>
 * In addition to serialization for checkpointing, RuleSets may read and write
 * themselves to streams in three ways.
 * 
 * <ul>
 * <li>
 * <b>writeRuleSet(...,DataOutput)/readRuleSet(...,DataInput)</b>&nbsp;&nbsp;
 * &nbsp;This method transmits or receives a RuleSet in binary. It is the most
 * efficient approach to sending RuleSets over networks, etc. The default
 * versions of writeRuleSet/readRuleSet reads/writes out the number of rules,
 * then calls read/writeRule(...) on each Rule. Override this if you need more
 * functionality.
 * 
 * <li>
 * <b>printRuleSet(...,PrintWriter)/readRuleSet(...,LineNumberReader)</b>&nbsp
 * ;&nbsp;&nbsp;This approach transmits or receives a RuleSet in text encoded
 * such that the RuleSet is largely readable by humans but can be read back in
 * 100% by ECJ as well. To do this, these methods will typically encode numbers
 * using the <tt>ec.util.Code</tt> class. These methods are mostly used to write
 * out populations to files for inspection, slight modification, then reading
 * back in later on. <b>readRuleSet</b> reads in the number of rules, then calls
 * readRule(...) on each new Rule. <b>printRuleSet</b> writes out the number of
 * rules, then calls printrule(...) on each new Rule. Again, override this if
 ...
 * <b>printRuleSetForHumans</b> prints out the number of rules, then calles
 * <b>printRuleForHumans</b> on each Rule in turn. You may wish to override this
 * to provide more information instead. You should handle one of these methods
 * properly to ensure RuleSets can be printed by ECJ.
 * </ul>
 * 
 * <p>
 * <b>Parameters</b><br>
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>constraints</tt><br>
 * <font size=-1>string</font></td>
 * <td valign=top>(name of the rule set constraints)</td>
 * </tr>
 * </table>
 * 
 * <p>
...
public class RuleSet implements Prototype {
    ...
    /*
     * Returns the RuleSet's constraints. A good JIT compiler should inline
     * this.
     */
    ...
    public void mutate(final EvolutionState state, final int thread) {
        ...
        while (state.random[thread].nextBoolean(constraints(initializer).p_del)
                && numRules > constraints(initializer).minSize) {
            removeRandomRule(state, thread);
        }
        while (state.random[thread].nextBoolean(constraints(initializer).p_add)
                && numRules < constraints(initializer).maxSize) {
    ...
    /**
     * Should be called by pipelines to "fix up" the rulesets before they have
     * been mutated or crossed over. Override this method to do so.
     */
    public void preprocessRules(final EvolutionState state, final int thread) {
    }

    /**
     * Should be called by pipelines to "fix up" the rulesets after they have
     * been mutated or crossed over. Override this method to do so.
     */
    ...
    /**
     * Removes a rule from the rule set and returns it. If index is out of
     * bounds, then this method returns null. The rules are shifted down ---
     * thus this is O(n).
     */
    public Rule removeRule(int index) {
        ...
        /*
         * // swap to the top Rule myrule = rules[index]; rules[index] =
         * rules[numRules-1];
         */
    ...
    /**
     * Removes a randomly-chosen rule from the rule set and returns it. If there
     * are no rules to remove, this method returns null.
     */
    public Rule removeRandomRule(final EvolutionState state, final int thread) {
        if (numRules <= 0)
            return null;
        else
            return removeRule(state.random[thread].nextInt(numRules));
    }
    ...
    /**
     * Clears out existing rules, and loads the rules from the other ruleset
     * without cloning them. Mostly for use if you create temporary rulesets
     * (see for example RuleCrossoverPipeline)
     */
    ...
    /**
     * Splits the rule set into n pieces, according to points, which *must* be
     * sorted. The rules in each piece are cloned and added to the equivalent
     * set. Sets must be already allocated. sets.length must be 1+
     * points.length. Comment: This function appends the split rulesets to the
     * existing rulesets already in <i>sets</i>.
     */
    ...
    /**
     * Splits the rule set into a number of disjoint rule sets, copying the
     * rules and adding them to the sets as appropriate. Each rule independently
     * throws a die to determine which ruleset it will go into. Sets must be
     * already allocated. Comment: This function appends the split rulesets to
     * the existing rulesets already in <i>sets</i>.
     */
    public RuleSet[] split(final EvolutionState state, final int thread, RuleSet[] sets) {
        for (int i = 0; i < numRules; i++)
            sets[state.random[thread].nextInt(sets.length)].addRule((Rule) (rules[i].clone()));
        return sets;
    }

    /**
     * Splits the rule set into a two disjoint rule sets, copying the rules and
     * adding them to the sets as appropriate. The value <i>prob</i> is the
     * probability that an element will land in the first set. Sets must be
     * already allocated. Comment: This function appends the split rulesets to
     * the existing rulesets already in <i>sets</i>.
     */
     ...
     */
    public void printRuleSetForHumans(final EvolutionState state, final int log, final int verbosity) {
        state.output.println("Ruleset contains " + numRules + " rules", log);
        for (int i = 0; i < numRules; i++) {
     ...
     * @deprecated Verbosity no longer has an effect
     */
    public void printRuleSet(final EvolutionState state, final int log, final int verbosity) {
     ...
     * Prints the rule set such that the computer can read it later
     */
    public void printRuleSet(final EvolutionState state, final PrintWriter writer) {
     ...
     * Reads the rule set
     */
    public void readRuleSet(final EvolutionState state, final LineNumberReader reader) throws IOException {
    ...
    public void writeRuleSet(final EvolutionState state, final DataOutput dataOutput) throws IOException {
    ...
    public void readRuleSet(final EvolutionState state, final DataInput dataInput) throws IOException {
    ...
    public void setup(EvolutionState state, Parameter base) {
        ...
        if (constraintname == null)
            state.output.fatal("No RuleSetConstraints name given", base.push(P_CONSTRAINTS),
    ...
    /**
     * The hash code for the rule set. This isn't a very good hash code, but it
     * has the benefit of not being O(n lg n) -- otherwise, we'd have to do
     * something like sort the rules in the individual first and then do an
     * ordered hash code of some sort, ick.
     */
    public int hashCode() {
    ...
    public boolean equals(final Object _other) {
        if (!getClass().equals(_other.getClass())) // not the same class, I'm
