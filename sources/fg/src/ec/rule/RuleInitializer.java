...
/**
 * A SimpleInitializer subclass designed to be used with rules. Basically, the
 * RuleInitializer sets up the RuleConstraints and RuleSetConstraints cliques at
 * setup() time, and does nothing else different from SimpleInitializer. The
 * RuleInitializer also specifies the parameter bases for the RuleSetConstraints
 ...
 * <table>
 * <tr>
 * <td valign=top><tt>rule.rsc</tt></td>
 * <td>RuleSetConstraints</td>
 * </tr>
 * <tr>
 * <td valign=top><tt>rule.rc</tt></td>
 * <td>RuleConstraints</td>
 * </tr>
 * </table>
 */
...
public class RuleInitializer extends SimpleInitializer {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        // will break.
        setupConstraints(state, RuleDefaults.base().push(P_RULECONSTRAINTS));
        setupRuleSetConstraints(state, RuleDefaults.base().push(P_RULESETCONSTRAINTS));
    }
    ...
    public void setupConstraints(final EvolutionState state, final Parameter base) {
        ...
        for (int y = 0; y < x; y++) {
            ...
            if (state.parameters.exists(base.push("" + y), null))
                c = (RuleConstraints) (state.parameters.getInstanceForParameterEq(base.push("" + y), null,
                        RuleConstraints.class));
            else {
                state.output
                        .message("No Rule Constraints specified, assuming the default class: ec.rule.RuleConstraints for "
                                + base.push("" + y));
    ...
    public void setupRuleSetConstraints(final EvolutionState state, final Parameter base) {
        ...
        for (int y = 0; y < x; y++) {
            ...
            if (state.parameters.exists(base.push("" + y), null))
                c = (RuleSetConstraints) (state.parameters.getInstanceForParameterEq(base.push("" + y), null,
                        RuleSetConstraints.class));
            else {
                state.output
                        .message("No RuleSetConstraints specified, assuming the default class: ec.gp.RuleSetConstraints for "
                                + base.push("" + y));
