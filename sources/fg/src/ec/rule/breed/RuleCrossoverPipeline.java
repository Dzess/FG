...
/**
 * 
 RuleCrossoverPipeline is a BreedingPipeline which implements a simple default
 * crossover for RuleIndividuals. Normally it takes two individuals and returns
 * two crossed-over child individuals. Optionally, it can take two individuals,
 * cross them over, but throw away the second child (a one-child crossover).
 * RuleCrossoverPipeline works by iteratively taking rulesets from each
 * individual, and migrating rules from either to the other with a certain
 * per-rule probability. Rule crossover preserves the min and max rule
 * restrictions.
 * 
 * <p>
 ...
 * <td valign=top>(probability that a rule will cross over from one individual
 * to the other)</td>
 * </tr>
 * </table>
 * 
...
public class RuleCrossoverPipeline extends BreedingPipeline {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		ruleCrossProbability = state.parameters.getFloatWithDefault(base.push(P_CROSSOVERPROB), def.push(P_CROSSOVERPROB), 0.5f);
...

	{
																...
																// filled up
		{
			...
			if (parents[0].rulesets.length != parents[1].rulesets.length) {
				state.output.fatal("The number of rule sets should be identical in both parents ( " + parents[0].rulesets.length + " : "
						+ parents[1].rulesets.length + " ).");
			}

			// for each set of rules (assume both individuals have the same
			// number of rule sets)
			for (int x = 0; x < parents[0].rulesets.length; x++) {
				...
				while (true) {
					...
					// split the ruleset indexed x in parent 2 (append after the
					// split results from previous operation)
