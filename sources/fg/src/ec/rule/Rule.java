...
/**
 * Rule is an abstract class for describing rules. It is abstract because it is
 * supposed to be extended by different classes modelling different kinds of
 * rules. It provides the reset abstract method for randomizing the individual.
 * It also provides the mutate function for mutating an individual rule It also
 * provides the clone function for cloning the rule.
 * 
 * <p>
 * You will need to implement some kind of artificial ordering between rules in
 * a ruleset using the Comparable interface, so the ruleset can be sorted in
 * such a way that it can be compared with another ruleset for equality. You
 * should also implement hashCode and equals in such a way that they aren't
 * based on pointer information, but on actual internal features.
 * 
 * <p>
 * Every rule points to a RuleConstraints which handles information that Rule
 * shares with all the other Rules in a RuleSet.
 * 
 * <p>
 * In addition to serialization for checkpointing, Rules may read and write
 * themselves to streams in three ways.
 * 
 * <ul>
 * <li>
 * <b>writeRule(...,DataOutput)/readRule(...,DataInput)</b>&nbsp;&nbsp;&nbsp;
 * This method transmits or receives a Rule in binary. It is the most efficient
 * approach to sending Rules over networks, etc. The default versions of
 * writeRule/readRule throw errors. You don't need to implement them if you
 * don't plan on using read/writeRule.
 * 
 * <li>
 * <b>printRule(...,PrintWriter)/readRule(...,LineNumberReader)</b>&nbsp;&nbsp
 * ;&nbsp;This approach transmits or receives a Rule in text encoded such that
 * the Rule is largely readable by humans but can be read back in 100% by ECJ as
 * well. To do this, these methods will typically encode numbers using the
 ...
 * in later on. <b>readRule</b> reads in a line, then calls
 * <b>readRuleFromString</b> on that line. You are responsible for implementing
 * readRuleFromString: the Code class is there to help you. The default version
 * throws an error if called. <b>printRule</b> calls <b>printRuleToString<b> and
 * printlns the resultant string. You are responsible for implementing the
 * printRuleToString method in such a way that readRuleFromString can read back
 * in the Rule println'd with printRuleToString. The default form of
 * printRuleToString() simply calls <b>toString()</b> by default. You might
 * override <b>printRuleToString()</b> to provide better information. You are
 * not required to implement these methods, but without them you will not be
 * able to write Rules to files in a simultaneously computer- and human-readable
 * fashion.
 * 
 * <li><b>printRuleForHumans(...,PrintWriter)</b>&nbsp;&nbsp;&nbsp;This approach
 * prints a Rule in a fashion intended for human consumption only.
 * <b>printRuleForHumans</b> calls <b>printRuleToStringForHumans()<b> and
 ...
 * ensure Rules can be printed by ECJ.
 * </ul>
 * 
 * <p>
 * <b>Parameters</b><br>
 ...
 * <td valign=top>(name of the rule constraint)</td>
 * </tr>
 * </table>
 * 
 * <p>
