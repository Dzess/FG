...
/**
 * This subclass of SimpleEvaluator evaluates the population, then computes
 * auxiliary fitness data of each subpopulation.
 */
...
public class SPEA2Evaluator extends SimpleEvaluator {
    ...
    /**
     * Computes the strength of individuals, then the raw fitness (wimpiness)
     * and kth-closest sparsity measure. Finally, computes the final fitness of
     * the individuals.
     */
    public void computeAuxiliaryData(EvolutionState state, Individual[] inds) {
        ...
        for (int y = 0; y < inds.length; y++) {
            ...
            for (int z = 0; z < inds.length; z++)
                if (((SPEA2MultiObjectiveFitness) inds[y].fitness)
                        .paretoDominates((MultiObjectiveFitness) inds[z].fitness))
                    myStrength++;
            ((SPEA2MultiObjectiveFitness) inds[y].fitness).strength = myStrength;
        } // For each individual y calculate the strength

        // calculate k value
        int kTH = (int) Math.sqrt(inds.length); // note that the first element
                                                // is k=1, not k=0
        ...
        for (int y = 0; y < inds.length; y++) {
            ...
            for (int z = 0; z < inds.length; z++) {
                // Raw fitness
                if (((SPEA2MultiObjectiveFitness) inds[z].fitness)
                        .paretoDominates((MultiObjectiveFitness) inds[y].fitness)) {
    ...
    /**
     * Returns a matrix of sum squared distances from each individual to each
     * other individual.
     */
    public double[][] calculateDistances(EvolutionState state, Individual[] inds) {
        ...
        for (int y = 0; y < inds.length; y++) {
            ...
            for (int z = y + 1; z < inds.length; z++) {
                distances[z][y] = distances[y][z] = ((SPEA2MultiObjectiveFitness) inds[y].fitness)
    ...
    /**
     * Returns the kth smallest element in the array. Note that here k=1 means
     * the smallest element in the array (not k=0). Uses a randomized sorting
     * technique, hence the need for the random number generator.
     */
    ...
    /*
     * OrderStatistics [Cormen, p187]: find the ith smallest element of the
     * array between indices p and r
     */
